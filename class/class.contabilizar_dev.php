<?php
	...
	class Contabilizar{
		...
		private function CalcularCostosProductosActaInternacional($productos, $tasa_dolar){
			...
			// var_dump($productos_factura);
			// var_dump($gastos_productos);
			// exit;
		...
		private function GetProductosFacturaCapita($idFactura){
			...
			P.Gravado,
			IFNULL((SELECT AVG(PAR.Precio) FROM Producto_Acta_Recepcion PAR WHERE PAR.Id_Producto = PD.Id_Producto AND PAR.Precio > 0), (SELECT AVG(Inve.Costo) FROM Inventario Inve WHERE Inve.Id_Producto = PD.Id_Producto AND Inve.Costo > 0 AND Inve.Id_Bodega != 0)) AS Costo
			FROM
