...
class FacturaElectronica
{
    ...
    function GenerarFactura()
    {
        ...
        if ($estado == "true") {
            ...
            // Prepara variables para los scripts que generan el PDF usando include local
            $originalGet = $_GET;
            $originalRequest = $_REQUEST;
            $_GET['id'] = $this->id_factura;
            $_GET['Ruta'] = $ruta_fact;
            $_REQUEST['id'] = $this->id_factura;
            $_REQUEST['Ruta'] = $ruta_fact;
            $pdfBasePath = $_SERVER['DOCUMENT_ROOT'] . "/php/facturacion_electronica/";
            
            if ($this->tipo_factura == "Factura") {
                include($pdfBasePath . 'factura_np_pdf.php');
            } elseif ($this->tipo_factura == "Factura_Venta") {
                include($pdfBasePath . 'factura_venta_pdf.php');
            } elseif ($this->tipo_factura == "Factura_Capita") {
                include($pdfBasePath . 'factura_capita_pdf.php');
            } elseif ($this->tipo_factura == "Factura_Administrativa") {
                include($pdfBasePath . 'factura_administrativa_pdf.php');
            }
