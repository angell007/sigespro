<?php
	class FileStorer{
		...
		private $https = 'https://';
		private $dominio = 'sigesproph.com.co';
		//private $dominio = '181.225.73.37';
		...
		function __destruct(){
			...
			$this->ruta_servidor_remoto_webservice = '';
			unset($this->credenciales_servidor);
			unset($this->ruta_servidor_remoto_webservice);
		}
		...
		public function UploadFileToRemoteServer2($files, $archivo_carga_remota, $ruta){
			global $MY_FILE;
		...
		public function MirrorFileUploadToRemoteServer($full_route, $fileName, $ruta){

			$result = '';

			$archivos = array('folders'=>$ruta);
			
			$output = new CURLFile('../ARCHIVOS/DISPENSACION/ACTAS_ENTREGAS/'.$fileName, 'application/pdf', $fileName);
			$archivos[] = $output;
				
			$curl = curl_init($full_route);
			curl_setopt($curl, CURLOPT_SAFE_UPLOAD, true);
			curl_setopt($curl, CURLOPT_HTTPHEADER , array('Content-Type: multipart/form-data;'));
			curl_setopt($curl, CURLOPT_POST, true);
			curl_setopt($curl, CURLOPT_POSTFIELDS, $archivos);
