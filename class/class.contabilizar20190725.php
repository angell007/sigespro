<?php
	...
	class Contabilizar{
		...
		private function CrearMovimientosFacturaMultitipo($datos){
			...
			if ($facturaObj['Tipo_Dispensacion'] != 'Evento' && $facturaObj['Tipo_Dispensacion'] != 'Cohortes') {

				$tipo_factura = 'Factura '.$this->GetTipoServicio($facturaObj['Tipo_Servicio']);
			}
		...
		private function CrearMovimientosDevolucionActa($datos){
			...
			$this->GetMovimientosActa($datos['Id_Acta']);
		}
			...
			switch ($tipo) {
				case 'acta':
					...
					foreach ($totales_facturas['Total_Facturas'] as $fact => $total) {
						...
						$oItem->Debe = "0";
						$oItem->Debe_Niif = "0";
						$oItem->Haber = number_format(floatval($total),2,".","");
						$oItem->Haber_Niif = number_format(floatval($total),2,".","");
						$oItem->Nit = $this->nit;
					...
					foreach ($totales_facturas['Facturas_19'] as $fact => $iva_total) {
						...
						$oItem->Id_Modulo = $this->id_modulo;
						$oItem->Id_Registro_Modulo = $this->id_registro_modulo;
						$oItem->Debe = number_format(floatval($iva_total),2,".","");
						$oItem->Debe_Niif = number_format(floatval($iva_total),2,".","");
						$oItem->Haber = "0";
						$oItem->Haber_Niif = "0";
						$oItem->Nit = $this->nit;
						$oItem->Tipo_Nit = $this->tipo_nit;
						...
						foreach ($factura['Retenciones'] as $rt) {
							...
							if ($rt['Valor'] > 0) {
								if ($rt['Tipo'] == 'Renta') {
									...
									$oItem->Debe = "0";
									$oItem->Debe_Niif = "0";
									$oItem->Haber = number_format(floatval($rt['Valor']),2,".","");
									$oItem->Haber_Niif = number_format(floatval($rt['Valor']),2,".","");
									$oItem->Nit = $this->nit;
									$oItem->Tipo_Nit = $this->tipo_nit;
		...
		private function GuardarMovimientoComprobanteIngreso($datos, $codigo_comprobante){
			...
			if (count($datos['Facturas']) > 0) {
				...
				$totales_facturas = $this->GetTotalesComprobante($datos['Facturas'], 'Ingreso', 'Facturas');
			}else{

				$totales_facturas = $this->GetTotalesComprobante($datos['Valores_Comprobante'], 'Ingreso', 'Comprobante');
			}
		...
		if (count($datos['Facturas']) > 0) {
			...
			if ($is_nota_credito !== false) {
				...
				$oItem->Debe = "0";
				$oItem->Haber = number_format($fact['neto'], 2, ".", "");
				$oItem->Debe_Niif = "0";
				$oItem->Haber_Niif = number_format($fact['neto'], 2, ".", "");
			}
			$oItem->Nit = $this->nit;
			$oItem->Tipo_Nit = $this->tipo_nit;
			...
			//GUARDAR MOV. DE RETENCIONES
					...
					if (count($datos['Facturas']) > 0) {
						...
						foreach ($fact['retenciones'] as $key => $value) {
							...
							if ($is_nota_credito !== false) {
								...
								$oItem->Haber = "0";
								$oItem->Debe = number_format($value['Total'], 2, ".", "");
								$oItem->Haber_Niif = "0";
								$oItem->Debe_Niif = number_format($value['Total'], 2, ".", "");
							}
							$oItem->Nit = $this->nit;
							$oItem->Tipo_Nit = $this->tipo_nit;
							$oItem->Documento = $fact['codigo'];
				...
				if ($fact['ajuste'] > 0) {
					...
					$oItem->Id_Modulo = $this->id_modulo;
					$oItem->Id_Registro_Modulo = $this->id_registro_modulo;
					if ((strpos($fact['codigo_ajuste'],'AJUSTE') !== false && strpos($fact['codigo_ajuste'],'5305') !== false) || strpos($fact['codigo_ajuste'],'ESTAMPILLA') !== false) {
						if ($is_nota_credito !== false) {
							$oItem->Debe = "0";
							...
							$oItem->Haber = "0";
							$oItem->Debe = number_format($fact['ajuste'], 2, ".", "");
							$oItem->Haber_Niif = "0";
							$oItem->Debe_Niif = number_format($fact['ajuste'], 2, ".", "");
						}
					} else {
						if ($is_nota_credito !== false) {
							$oItem->Haber = "0";
			...
			foreach ($totales_facturas['Retenciones'] as $key => $value) {
				$asociacion = $this->BuscarInformacionParaMovimiento($key);
		...
		private function GuardarMovimientosActaInternacional($datos, $tasa){
			...
			foreach ($productos_totales as  $key => $value) {
				...
				if ($key == 'excluidos') {
					if ($value > 0) {
						...
						if ($this->save_fecha)
							$oItem->Fecha_Movimiento = $this->GetFechaMovimiento($this->id_registro_modulo, 'Acta Internacional');
						$oItem->save();
						unset($oItem);
					}					
		...
		private function CalcularCostosProductosActaInternacional($productos, $tasa_dolar){
			...
			$total_gastos_productos = 0;
			$gastos_productos = array('excluidos' => 0, 'gravados' => 0, 'total' => 0);
			...
			foreach ($productos as $p) {
				...
				foreach ($p['Producto_Lotes'] as $lote) {
					...
					$total_gastos_productos += floatval($lote['Subtotal']) * floatval($tasa_dolar);
					
					if ($p['Gravado'] == 'No') {
						$gastos_productos_excluidos += floatval($lote['Subtotal']) * floatval($tasa_dolar);
					}else{
						$gastos_productos_gravados += floatval($lote['Subtotal']) * floatval($tasa_dolar);
					}
				}				
			...
			$gastos_productos['total'] = $total_gastos_productos;
		...
		private function CrearMovimientosParcialActaInternacional($datos){
			...
			foreach ($diferencia_cambio as $key => $value) {
				...
				if ($key == 'total_excento_recalculado') {
					if ($value != 0) {
						...
						if ($this->save_fecha)
							$oItem->Fecha_Movimiento = $this->GetFechaMovimiento($this->id_registro_modulo, 'Parcial Acta Internacional');
						$oItem->save();
						unset($oItem);
			...
			if ($datos['Modelo']['Descuento_Parcial'] != '0') {
				...
				$oItem->Debe = "0";
				$oItem->Debe_Niif = "0";
				$oItem->Nit = trim(str_replace(".", "", $this->GetNitProh()), "-5");
			...
			if ($diferencia_cambio['cuenta'] == 'diferencia ingreso') {
				$oItem->Haber = "0";
				$oItem->Haber_Niif = "0";
				$oItem->Debe = round(number_format($diferencia_cambio['diferencia'], 2, ".", ""));
				$oItem->Debe_Niif = round(number_format($diferencia_cambio['diferencia'], 2, ".", ""));
			}else{
				$oItem->Debe = "0";
				$oItem->Debe_Niif = "0";
				$oItem->Haber = round(number_format($diferencia_cambio['diferencia'], 2, ".", ""));
				$oItem->Haber_Niif = round(number_format($diferencia_cambio['diferencia'], 2, ".", ""));
			}
			$oItem->Nit = trim(str_replace(".", "", $this->GetNitProh()), "-5");
			$oItem->Tipo_Nit = 'Empresa';
			$oItem->Documento = $datos['Modelo']['Codigo'];
		...
		private function GetNitGastosActa($id_acta){
			...
			$queryObj->SetQuery($query);
			$acta_data = $queryObj->ExecuteQuery('simple');
			return $acta_data;
		}
		...
		private function GetNitProh(){
			...
			$queryObj->SetQuery($query);
			$nit = $queryObj->ExecuteQuery('simple');
			return $nit['NIT'];
		}
		...
		private function GetNitProveedorExtranjero($id_acta){
			...
			$queryObj->SetQuery($query);
			$proveedor = $queryObj->ExecuteQuery('simple');
		...
		private function GetDatosActaInternacional($id_acta){
			...
			$queryObj->SetQuery($query);
			$datos_acta = $queryObj->ExecuteQuery('multiple');
			return $datos_acta;
		}
		...
		private function TotalIvaProductos($productos, $tipo_calculo, $nroFactura = '', $cuota_moderadora = 0, $clase_ajuste = null){
			...
			if ($tipo_calculo == 'ajuste Entrada' || $tipo_calculo == 'ajuste Salida') {
				...
				foreach ($gravados_cargar as $key => $value) {
					...
					$oItem->Id_Modulo = $this->id_modulo;
					$oItem->Id_Registro_Modulo = $this->id_registro_modulo;
					$oItem->Debe = $tipo_calculo == 'ajuste Entrada' ? number_format($total_cargar,2,".","") : "0";
					$oItem->Haber =  $tipo_calculo == 'ajuste Entrada' ? "0" : number_format($total_cargar,2,".","");
					$oItem->Debe_Niif = $tipo_calculo == 'ajuste Entrada' ? number_format($total_cargar,2,".","") : "0";
					$oItem->Haber_Niif =  $tipo_calculo == 'ajuste Entrada' ? "0" : number_format($total_cargar,2,".","");
					$oItem->Nit = $this->nit;
					$oItem->Tipo_Nit = $this->tipo_nit;
					$oItem->Documento = $nroFactura;
					$oItem->Numero_Comprobante = $nroFactura;
					...
					$oItem->Id_Modulo = $this->id_modulo;
					$oItem->Id_Registro_Modulo = $this->id_registro_modulo;
					$oItem->Debe = $tipo_calculo == 'ajuste Entrada' ? "0" : number_format($total_cargar,2,".","");
					$oItem->Haber = $tipo_calculo == 'ajuste Entrada' ? number_format($total_cargar,2,".","") : "0";
					$oItem->Debe_Niif = $tipo_calculo == 'ajuste Entrada' ? "0" : number_format($total_cargar,2,".","");
					$oItem->Haber_Niif = $tipo_calculo == 'ajuste Entrada' ? number_format($total_cargar,2,".","") : "0";
					$oItem->Nit = $this->nit;
					$oItem->Tipo_Nit = $this->tipo_nit;
					$oItem->Documento = $nroFactura;
					$oItem->Numero_Comprobante = $nroFactura;
		...
		private function GetTipoFactura($idFactura){
			$query = '
				SELECT
					F.*,
					D.Tipo AS Tipo_Dispensacion,
					IFNULL(D.Tipo_Servicio, "No aplica") AS Tipo_Servicio
				FROM Factura F
		...
		private function GetProductosFacturaCapita($idFactura){
			...
			WHERE D.Tipo = "Capita" AND D.Id_Factura = '.$idFactura;
		...
		public function AnularMovimientoContable($idRegistroModulo, $idModulo){
			$query_anular = 'UPDATE Movimiento_Contable SET Estado = "Anulado" WHERE Id_Registro_Modulo IN ('.$idRegistroModulo.') AND Id_Modulo = '.$idModulo;
