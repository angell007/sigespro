...
class CalculoNomina
{
    ...
    function __construct($funcionario, $quincena, $fini, $ffin, $tipo = null, $nom = "Mensual",$estado= "Activo")
    {
    ...
    public function CalculosNomina()
    {
        ...
        $quin['DetalleEgresos'] = $this->ObtenerDetalleEgresos();
    ...
    private function CalcularSueldo()
    {
        ...
        if (($dias_trabajados >= 29) && $this->nom == 'Mensual') {
            $dias_trabajados = 30;
        }
    ...
    private function GetSalarioFuncionario()
    {
         $query = "SELECT CF.*,
                    ...
                    (SELECT Valor_Unidad_Tributaria FROM Configuracion WHERE Id_Configuracion=1 ) as Valor_Uvt, 
                    (SELECT Salario_Auxilio_Transporte FROM Configuracion WHERE Id_Configuracion=1 ) as Salario_Auxilio_Transporte,
                    IFNULL((SELECT Salario FROM Otrosi_Contrato WHERE Id_Contrato_Funcionario=CF.Id_Contrato_Funcionario AND Estado='Activo' ORDER BY Id_Otrosi_Contrato DESC LIMIT 1 ),CF.Valor) as Valor 
                  FROM Contrato_Funcionario CF
                  WHERE CF.Estado LIKE '$this->estado' AND CF.Identificacion_Funcionario= '$this->funcionario'";
    ...
    private function GetDatosFuncionario()
    {
        ...
        INNER JOIN Concepto_Parametro_Nomina CPN ON ME.Id_Tipo=CPN.Id_Concepto_Parametro_Nomina 
        INNER JOIN  Parametro_Nomina PN ON PN.Id_Parametro_Nomina=CPN.Id_Parametro_Nomina
        WHERE ME.Identificacion_Funcionario=F.Identificacion_Funcionario AND ME.Tipo="Ingreso" AND PN.Tipo="No_Prestacional" AND ME.Quincena LIKE "' . date("Y-m;", strtotime($this->fini)) . $this->quincena . '"),0)+' . $bonificacion .
            ' as Ingresos_NS,
    ...
    private function GetIngresosNoSalariales()
    {
        ...
        // if($this->funcionario=='1022416855'){
        //     echo $query; exit;
    ...
    private function GetEgresos()
    {
       ...
       $query = 'SELECT CPN.*, IFNULL(ME.Valor, 0) AS Valor, ME.Id_Movimiento_Funcionario
          FROM Parametro_Nomina PN
          INNER JOIN Concepto_Parametro_Nomina CPN ON PN.Id_Parametro_Nomina = CPN.Id_Parametro_Nomina
          LEFT JOIN Movimiento_Funcionario ME ON CPN.Id_Concepto_Parametro_Nomina = ME.Id_Tipo 
          AND ME.Tipo = "Egreso" 
          AND ME.Identificacion_Funcionario = ' . $this->funcionario . ' 
          AND ME.Quincena LIKE "' . date("Y-m", strtotime($this->fini)) . '%"
          WHERE PN.Tipo = "Deduccion"
          LIMIT 0, 25;';
    ...
    private function GetNovedades()
    {
        ...
        foreach ($datos as $nov) {
            ...
            $dias_nov = $dias_nov + 1;
            $datos[$y]["Dias"] = $dias_nov;
            $this->dias_ausente += $dias_nov;
            $dias = round((strtotime($nov['Fecha_Fin']) - strtotime($nov['Fecha_Inicio'])) / 86400);
            $dias = $dias + 1;
            if ($nov["Tipo_Novedad"] == "Vacaciones" && $nov["Fecha_Inicio"] >= $this->fini) {
                $datos[$y]["Dias"] = $dias;
                $this->dias_vacaciones = $dias;
                $this->lista_vacaciones[] = $datos[$y];
                $this->total_vacaciones += ($this->valor_dia * $dias);
            }
                ...
                if ($nov["Novedad"] == 'Incapacidad general') {
                    ...
                    if ($dias_nov > 2 ) {
                        if( $this->ValidarIncapacidad() ){
                            ...
                            $dias_incapacidad = $dias_nov - 2;
                            $valor_dia = $this->salario_funcionario['Salario_Base'] / 30 ;
                            $this->total_incapacidades += (2 * $this->valor_dia) + (($dias_incapacidad * $valor_dia));
                            $incapacidad_general += (2 * $this->valor_dia) + (($dias_incapacidad * $valor_dia));
                        }
    ...
    private function ValidarFondoSolidaridad()
    {
        ...
        if ($this->salario_funcionario['Aporte_Pension'] == 'Si') {
            ...
            $query = "SELECT
            (CASE
                WHEN CHAR_LENGTH(Rango)=1 THEN IF($valor>=(Rango*$salario_base), Porcentaje*($valor),0)           
    ...
    private function CalcularTotalIngresos()
    {
        ...
        $total = (round($this->total_ibc, 0) + round($this->total_auxilio, 0) + round($bono, 0) + round($this->total_ingresosn_salarial, 0)) - ((int)$this->total_vacaciones);
    ...
    private function ObtenerDetalleEgresos()
{
    $detalleEgresos = [
        'Pensi¨®n' => round($this->deduccion_pension, 0),
        'Salud' => round($this->deduccion_salud, 0),
        'Fondo Solidaridad' => round($this->total_solidaridad, 0),
    ...
    private function ArmarIngresosConstitutivos()
    {
        ...
        $total = [[
            ...
            'Valor' => $this->salario_quincena + (int)$this->total_extras + (int)$this->total_vacaciones + (int)$this->total_incapacidades + (int)$this->total_licencias + (int)$this->datos_funcionario['Ingresos_S'] + (int)$this->prima
        ]];
    ...
    private function ArmarRetenciones($fondo_subsistencia, $fondo_solidaridad)
    {
        ...
        $salario = [
            [
                ...
                'IBC' => '$' . number_format($this->total_ibc, 0, "", "."),
                'Formula' => '$' . number_format($this->total_ibc, 0, "", ".") . ' * ' . ($this->porcentaje_salud * 100) . '%'
            ],
            [
                'Concepto' => 'Pensi¨®n',
                ...
                'IBC' => '$' . number_format($this->total_ibc, 0, "", "."),
                'Formula' => '$' . number_format($this->total_ibc, 0, "", ".") . ' * ' . ($this->porcentaje_pension * 100) . '%'
            ]
    ...
    private function GetRetenciones()
    {
        if ($this->quincena == 2 || $this->nom == "Mensual") {
            ...
            $total = ($this->total_ibc - (($this->deduccion_pension + $this->deduccion_salud + $this->total_solidaridad)));
            ...
            $uvt = str_replace(',', '.', round($ingreso_laboral_mesual_base / $this->salario_funcionario['Valor_Uvt'], 5));
            // echo $uvt; exit;

            // echo $this->salario_funcionario['Valor_Uvt']; exit;
