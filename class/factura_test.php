...
//require_once('./helper/factura_elec_dis_helper.php');
...
class FacturaElectronica
{
    ...
    function GenerarFactura()
    {
        ...
        if ($estado == "true") {
            ...
            if ($this->tipo_factura == "Factura") {
                include('https://192.168.40.201/php/facturacion_electronica/factura_np_pdf.php?id=' . $this->id_factura . '&Ruta=' . $ruta_fact);
            } elseif ($this->tipo_factura == "Factura_Venta") {
                include('https://192.168.40.201/php/facturacion_electronica/factura_venta_pdf.php?id=' . $this->id_factura . '&Ruta=' . $ruta_fact);
            } elseif ($this->tipo_factura == "Factura_Capita") {
                include('https://192.168.40.201/php/facturacion_electronica/factura_capita_pdf.php?id=' . $this->id_factura . '&Ruta=' . $ruta_fact);
            } elseif ($this->tipo_factura == "Factura_Administrativa") {
                include('https://192.168.40.201/php/facturacion_electronica/factura_administrativa_pdf.php?id=' . $this->id_factura . '&Ruta=' . $ruta_fact);
            }
        }
        ...
        if ($respuesta_dian["Estado"] == "error") {
            ...
            $respuesta["Respuesta_Correo"] = $this->EnviarMail($cufe, $qr, $respuesta_dian["Respuesta"]);
    ...
    private function EnviarMail($cufe, $qr, $dian)
    {
    ...
    {
        $xml = '/home/sigespro/api-dian.192.168.40.201/api-dian/storage/app/xml/1/' . $this->resolucion["resolution_id"] . '/fv' . $this->getNombre() . '.xml';
    ...
    public function ConsultaCufe($cufe){
 ...
 /*
    ...
    private function GeneraJson($tipo_factura)
    {
        ...
        $impues[0]["tax_id"] = 1;
        $impues[0]["tax_amount"] = number_format($tot_imp, 2, ".", "");
        $impues[0]["taxable_amount"] = number_format($base_imp, 2, ".", "");
        $impues[0]["percent"] = "19";
        ...
        switch ($facturaAdmin['Tipo_Cliente']) {
            case 'Funcionario':
                $query = 'SELECT "Funcionario" AS Tipo_Tercero, Identificacion_Funcionario AS Id_Cliente , "No" as Contribuyente, "No" as Autorretenedor,
                        ...
                        Id_Municipio, 1 AS Condicion_Pago
    ...
    private function getDataDis()
    {
       ...
       return ($this->tipo_factura == 'Factura' 
                    ? getDatosDisHelper($this->factura['Id_Dispensacion'],
                                     $this->configuracion,$this->cliente ) 
                    : "" );
