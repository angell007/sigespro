...
for ($i=0; $i < count($meses); $i++) { 
    ...
    $maximo_Factura='(SELECT F1.Codigo FROM Producto_Factura PF1 INNER JOIN Factura F1 ON PF1.Id_Factura=F1.Id_Factura '.$condicion.' AND F1.Tipo!="Homologo" ORDER BY PF1.Precio DESC LIMIT 1 ) as Maximo_Factura,';


    $minimo_Factura='( SELECT F1.Codigo FROM Producto_Factura PF1 INNER JOIN Factura F1 ON PF1.Id_Factura=F1.Id_Factura '.$condicion.' AND F1.Tipo!="Homologo" ORDER BY PF1.Precio ASC LIMIT 1 ) as Minimo_Factura,';
    
    $subtotal_Factura='(SELECT SUM(PF1.Precio*PF1.Cantidad) FROM Producto_Factura PF1 INNER JOIN Factura F1 ON PF1.Id_Factura=F1.Id_Factura '.$condicion.' ) as Precio,';
    
    $cantidad_Factura='(SELECT SUM(PF1.Cantidad) FROM Producto_Factura PF1 INNER JOIN Factura F1 ON PF1.Id_Factura=F1.Id_Factura '.$condicion.') as Cantidad,';
    ...
    $maximo_factura_venta='(SELECT F1.Codigo FROM Producto_Factura_Venta PF1 INNER JOIN Factura_Venta F1 ON PF1.Id_Factura_Venta=F1.Id_Factura_Venta '.$condicion.' ORDER BY PF1.Precio_Venta DESC LIMIT 1 ) as Maximo_Factura,';
    
    $minimo_factura_venta='(SELECT F1.Codigo FROM Producto_Factura_Venta PF1 INNER JOIN Factura_Venta F1 ON PF1.Id_Factura_Venta=F1.Id_Factura_Venta '.$condicion.' ORDER BY PF1.Precio_Venta ASC LIMIT 1 ) as Minimo_Factura,';
    
    $subtotal_factura_venta='(SELECT SUM(PF1.Precio_Venta*PF1.Cantidad) FROM Producto_Factura_Venta PF1 INNER JOIN Factura_Venta F1 ON PF1.Id_Factura_Venta=F1.Id_Factura_Venta '.$condicion.') as Precio,';
    
    $cantidad_factura_venta='(SELECT SUM(PF1.Cantidad) FROM Producto_Factura_Venta PF1 INNER JOIN Factura_Venta F1 ON PF1.Id_Factura_Venta=F1.Id_Factura_Venta '.$condicion.' ) as Cantidad,';
...
foreach($resultado as $item){ $j++;$i++;
	...
	$objSheet->getCell('J'.$j)->setValue($factura[1]);
    ...
    $texto.="2,".$i.",".$item["Mes"].",INS,".$item["Codigo_Cum"].",SI,".$item["Minimo"].",".$item["Maximo"].",".number_format($item["Precio"],2,".","").",".$item["Cantidad"].",".$item['Minimo_Factura'].",".$item['Maximo_Factura']."\r\n";
