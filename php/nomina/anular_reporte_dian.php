...
{
  $query = 'SELECT FN.Fecha, N.Tipo_Nomina 
    ...
    // echo json_encode($funcionario["Tipo_Nomina"]);exit;
    ...
    $data['cune_propio'] = cuneGenerate($data, '804016084', '00777', '103', 1, 2);//103 QUEMAO - NOM.AJUSTE
    ...
    $nominaElectronica = nominaElectronicaFuncionario($respuesta_dian,$data);
...
function nominaElectronicaFuncionario($respuesta_dian,$data)
{
    global $func;
    ...
    $oItem->Cune = $respuesta_dian['status'] == 'succeded' ?  $data['cune_propio'] : '';
    $oItem->Fecha_Reporte =date('Y-m-d');
    $oItem->Estado = 'Anulado';
    $oItem->Respuesta_Dian =$respuesta_dian["Respuesta"];
    $oItem->Codigo_Nomina=  $respuesta_dian['status'] == 'succeded' ?  $data['code'] : '';
    $oItem->Observacion= $datos["Motivo_Anulacion"];
    $oItem->Funcionario= $func;
    $oItem->save();
    $nominaElectronica = $oCon->getData();
    
    unset($oItem); 
    
    return $nominaElectronica;
}

function updateNominaFuncionario($data,$nominaElectronica)
{
