...
$oLista = new lista("Nomina_Funcionario");
$oLista->setRestrict("Id_Nomina","=",$nom);
...
$query = 'SELECT NF.*,  CONCAT(F.Nombres," ",F.Apellidos) as Funcionario,  (SELECT C.Nombre FROM Cargo C WHERE C.Id_Cargo=F.Id_Cargo) as Cargo, CF.Fecha_Inicio_Contrato as Fecha_Ingreso, CF.Valor as Salario
...
WHERE NF.Identificacion_Funcionario='.$fun["Identificacion_Funcionario"].' AND NF.Id_Nomina_Funcionario='.$fun["Id_Nomina_Funcionario"];
...
$query="SELECT MN.* FROM Movimiento_Nomina_Funcionario MN WHERE MN.Id_Nomina_Funcionario= ".$fun["Id_Nomina_Funcionario"];
...
unset($oCon);
       ...
       $contenido = '<table style="border:1px solid #cccccc;"  cellpadding="0" cellspacing="0">
           <tr style="width:590px;" >
                           ...
                           <td   style="width:150px;font-size:10px;font-weight:bold;text-align:center;background:#ededed;border:1px solid #cccccc;padding:4px;padding-right:2;">Quincena</td>
       ...
       <table style="font-size:10px;margin-top:10px;" cellpadding="0" cellspacing="0">
           ...
           foreach ($conceptos as $value) {
            if((INT)$value["Valor"]!=0){
                $contenido.='
                <tr >
                      <td style="width:580px;max-width:400px;border:1px solid #cccccc; max-height:50px;padding:4px;'.($value["Concepto"]=="Total neto a pagar al empleado" ? 'font-weight:bold;' : '').'">
                      '.$value['Concepto'].'
                      </td>
                      <td style="text-align:right;width:150px;max-width:100px;border:1px solid #cccccc;padding:4px;'.($value["Concepto"]=="Total neto a pagar al empleado" ? 'font-weight:bold;' : '').'">$ '.number_format($value['Valor'],2,".",",").'
                      </td>
                </tr>'; 
            }
           }
