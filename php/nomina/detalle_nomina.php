...
$fecha=explode(";",$nomina['Nomina']);
$mes=explode("-",$fecha[0]);
$mes1=MesString($mes[1]);
$nomina['Nomina_Fecha']=$fecha[1]." Quincena de ".$mes1." del ".$mes[0];

$query2 = 'SELECT COUNT(*) as Total
FROM Nomina_Funcionario NF INNER JOIN Funcionario F ON NF.Identificacion_Funcionario=F.Identificacion_Funcionario WHERE NF.Id_Nomina='.$id.$condicion;     
...
FROM Nomina_Funcionario NF INNER JOIN Funcionario F ON NF.Identificacion_Funcionario=F.Identificacion_Funcionario WHERE NF.Id_Nomina='.$id.$condicion.' LIMIT '.$limit.','.$tamPag;   
$oCon= new consulta();
