...
$id_zona=(isset( $_REQUEST['id'] ) ? $_REQUEST['id'] : false);
...
$query = "SELECT 
      ...
      Inner Join (
            SELECT SUM(PFV.Subtotal) AS Ventas, C.Id_Cliente, Ifnull(SUM(NC.Nota),0)+ ifnull(SUM(NG.Nota),0)AS Notas
                  ...
                  WHERE MONTH(FV.Fecha_Documento) = MONTH(NOW()) AND YEAR(FV.Fecha_Documento)= YEAR(NOW()) AND FV.Estado != 'Anulada' 
                  GROUP BY C.Id_Cliente
            ) Rep on Rep.Id_Cliente= C.Id_Cliente
      WHERE MONTH(FV.Fecha_Documento) = MONTH(NOW())  AND FV.Estado != 'Anulada' AND YEAR(FV.Fecha_Documento)=YEAR(NOW()) AND C.Id_Zona=$id_zona GROUP BY C.Id_Cliente Order BY  Ventas DESC";
...
$query = "SELECT ZC.Nombre FROM Zona ZC WHERE ZC.Id_Zona= ".$id_zona;


$oCon = new consulta();
$oCon->setQuery($query);
$resultado['Zona'] = $oCon->getData();
unset($oCon);
