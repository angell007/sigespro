...
$solo_diferencias = isset($_GET['solo_diferencias']) ? (int) $_GET['solo_diferencias'] : 1;
$apply = isset($_GET['apply']) ? (int) $_GET['apply'] : 0;
...
<head>
    ...
    <title>Ajuste Huérfanos - Inventario</title>
    <style>
        :root {
            --bg: #0b0f14;
            --card: #121a22;
...
<body>
    <header>
        <h1>Ajuste de Cantidades Huérfanas</h1>
        <div class="sub">Punto: <?php echo h($id_punto); ?> · Rango Kardex: <?php echo h($fecha_inicio); ?> → <?php echo h($fecha_fin); ?> · <?php echo $apply ? '<span class="pill">APLICADO</span>' : '<span class="pill">SIMULACIÓN</span>'; ?></div>
    </header>
...
foreach ($registros as $reg) {
    ...
    $nuevo = $saldo_kardex < 0 ? 0 : $saldo_kardex;
    $diferencia = $nuevo - (float) $reg['Cantidad'];

    if ($solo_diferencias && abs($diferencia) < 0.0001) {
        continue;
    }

    $estado = abs($diferencia) < 0.0001 ? 'OK' : ($apply ? 'ACTUALIZADO' : 'PENDIENTE');
    $badge = $estado === 'OK' ? '<span class="badge ok">OK</span>' : ($apply ? '<span class="badge warn">UPD</span>' : '<span class="badge danger">DIF</span>');

    if ($apply && abs($diferencia) >= 0.0001) {
        $q = "UPDATE Inventario_Nuevo
              SET Cantidad = " . (float) $nuevo . "
              WHERE Id_Inventario_Nuevo = " . (int) $reg['Id_Inventario_Nuevo'];
        $oCon = new consulta();
        $oCon->setQuery($q);
        $oCon->createData();
        unset($oCon);
    ...
    echo '<tr class="' . (abs($diferencia) >= 0.0001 ? 'diff' : '') . '">
        <td>' . h($row_num) . '</td>
...
?>
    ...
    <div class="muted" style="padding: 0 24px 24px;">
        Actualizados: <?php echo h($actualizados); ?> (apply=<?php echo h($apply); ?>)
    </div>
</body>
</html>
