...
function getDatosTercero($nit) {
    ...
    $query = "
    (SELECT CONCAT_WS(' ', Nombres, Apellidos) AS Nombre, SUBSTRING_INDEX( trim(Nombres), ' ', 1) Primer_Nombre, TRIM(REPLACE(TRIM(Nombres), SUBSTRING_INDEX( trim(Nombres), ' ', 1), '')) Segundo_Nombre, SUBSTRING_INDEX( trim(Apellidos), ' ', 1) Primer_Apellido, TRIM(REPLACE(TRIM(Apellidos), SUBSTRING_INDEX( trim(Apellidos), ' ', 1), '')) Segundo_Apellido, Direccion_Residencia AS Direccion, M.Dep_Cod_Dian, M.Cod_Municipio FROM Funcionario F INNER JOIN Contrato_Funcionario FC ON F.Identificacion_Funcionario = FC.Identificacion_Funcionario INNER JOIN( SELECT T.Id_Municipio, T.Codigo_Dane AS Cod_Municipio, ( SELECT Codigo FROM Departamento WHERE Id_Departamento = T.Id_Departamento ) AS Dep_Cod_Dian FROM Municipio T )M ON FC.Id_Municipio = M.Id_Municipio WHERE F.Identificacion_Funcionario = $nit)
    UNION
    (SELECT IF(Primer_Nombre != '',CONCAT_WS(' ',Primer_Nombre,Segundo_Nombre,Primer_Apellido,Segundo_Apellido),Razon_Social) AS Nombre, Primer_Nombre, Segundo_Nombre, Primer_Apellido, Segundo_Apellido, Direccion, (SELECT Codigo FROM Departamento WHERE Id_Departamento = C.Id_Departamento) AS Dep_Cod_Dian, (SELECT Codigo_Dane FROM Municipio WHERE Id_Municipio = C.Id_Municipio) AS Cod_Municipio FROM Cliente C WHERE Id_Cliente = $nit)
    UNION
