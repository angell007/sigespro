...
/* HAGO UN SWITCH PARA TODOS LOS MODULOS QUE PUEDEN GENERAR PDF */
        ...
        $query = "SELECT PC.Codigo, PC.Nombre AS Cuenta, PC.Nombre_Niif AS Cuenta_Niif, PC.Codigo_Niif, CNC.Concepto, CNC.Documento, CNC.Nit, (
            ...
            END
            ) AS Tercero, (SELECT IFNULL(Nombre,'Sin Centro Costo') FROM Centro_Costo WHERE Id_Centro_Costo = CNC.Id_Centro_Costo) AS Centro_Costo, CNC.Debito, CNC.Credito, CNC.Cred_Niif, CNC.Deb_Niif FROM Cuenta_Documento_Contable CNC INNER JOIN Plan_Cuentas PC ON CNC.Id_Plan_Cuenta = PC.Id_Plan_Cuentas WHERE CNC.Id_Documento_Contable = $id";
