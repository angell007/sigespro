...
/* HAGO UN SWITCH PARA TODOS LOS MODULOS QUE PUEDEN GENERAR PDF */
        ...
        FROM Nota_Contable NC WHERE NC.Estado = 'Activo' AND NC.Id_Nota_Contable = $id";
        ...
        $query = "SELECT PC.Codigo, PC.Nombre AS Cuenta, CNC.Concepto, CNC.Documento, CNC.Nit, CNC.Debito, CNC.Credito FROM Cuenta_Nota_Contable CNC INNER JOIN Plan_Cuentas PC ON CNC.Id_Plan_Cuenta = PC.Id_Plan_Cuentas WHERE CNC.Id_Nota_Contable = $id";
    ...
    foreach ($cuentas as $cuenta) {
        ...
        <td style="text-align:right;width:84px;padding:4px;border:1px solid #cccccc;">
            $.'.number_format($cuenta['Debito'],2,'.',',').'
        </td>
        <td style="text-align:right;width:84px;padding:4px;border:1px solid #cccccc;">
            $.'.number_format($cuenta['Credito'],2,'.',',').'
        </td>
    </tr>';

    $totalDeb += $cuenta['Debito'];
    $totalCred += $cuenta['Credito'];
