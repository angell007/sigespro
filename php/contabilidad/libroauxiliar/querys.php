...
function getStrCondiciones()
{
    ...
    if (isset($cuenta_inicial) && $cuenta_final != '') {
        $condicion .= " WHERE $column BETWEEN '$cuenta_inicial' AND '$cuenta_final'";
    }
...
function queryByNit($id_plan_cuenta, $inicial = false)
{
    global $ultimo_dia_mes;
    ...
    if (isset($_REQUEST['Nit']) && $_REQUEST['Nit'] != '') {
        $condicion = "  WHERE MC.Nit = $_REQUEST[Nit]";
        
    }

    if (!$inicial) {
        $query = "SELECT MC.Nit, (CASE MC.Tipo_Nit
        WHEN 'Cliente' THEN (SELECT IF(CONCAT_WS(' ',Primer_Nombre,Segundo_Nombre,Primer_Apellido,Segundo_Apellido) != '', CONCAT_WS(' ',Primer_Nombre,Segundo_Nombre,Primer_Apellido,Segundo_Apellido),Razon_Social) FROM Cliente WHERE Id_Cliente = MC.Nit Limit 1)
        WHEN 'Proveedor' THEN (SELECT IF(CONCAT_WS(' ',Primer_Nombre,Segundo_Nombre,Primer_Apellido,Segundo_Apellido) != '', CONCAT_WS(' ',Primer_Nombre,Segundo_Nombre,Primer_Apellido,Segundo_Apellido),Razon_Social) FROM Proveedor WHERE Id_Proveedor = MC.Nit Limit 1)
        WHEN 'Funcionario' THEN (SELECT CONCAT_WS(' ', Nombres, Apellidos) FROM Funcionario WHERE Identificacion_Funcionario = MC.Nit Limit 1)
        WHEN 'Eps' THEN (SELECT Nombre FROM Eps WHERE Nit = MC.Nit Limit 1)
        WHEN 'Arl' THEN (SELECT Nombre FROM Arl WHERE Nit = MC.Nit  Limit 1)
        WHEN 'ICBF' THEN 'ICBF'
        WHEN 'Sena' THEN 'SENA'
        WHEN 'Caja_Compensacion' THEN (SELECT Nombre FROM Caja_Compensacion WHERE Nit = MC.Nit  Limit 1)
        WHEN 'Fondo_Pension' THEN (SELECT Nombre FROM Fondo_Pension WHERE Nit = MC.Nit Limit 1)
    ...
    END) AS Nombre_Nit, (SELECT IFNULL(SUM(Debito_PCGA),0) FROM Balance_Inicial_Contabilidad WHERE Fecha = '$ultimo_dia_mes' AND Nit = MC.Nit AND Id_Plan_Cuentas = $id_plan_cuenta) AS Debito_PCGA,
        ...
        FROM Movimiento_Contable MC $condicion AND MC.Id_Plan_Cuenta = $id_plan_cuenta GROUP BY MC.Nit ORDER BY MC.Fecha_Movimiento, MC.Numero_Comprobante ASC";
    } else {
