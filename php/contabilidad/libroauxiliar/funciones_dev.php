  ...
  function obtenerSaldoAnterior($naturaleza, $array, $index, $tipo_nit = false)
{
    ...
    // // echo "$array[$index][Debito_$tipo_reporte] - $array[$index][Credito_$tipo_reporte]";
    // exit;
    ...
    echo '</pre>';*/
//var_dump($naturaleza,$array,$index,$array[$index]["Debito_$tipo_reporte"] ,  $array[$index]["Credito_$tipo_reporte"], $saldo_anterior);exit;
    ...
    if ($fecha1 != '2019-01-01') {
       /*  // $fecha1 = date('Y-01-01', strtotime($fecha_ini)); // Primer día del mes de enero
        ...
        $saldos = $tipo_nit ? getMovimientosSaldoCuenta($fecha1,$fecha2,$array[$index]["Id_Plan_Cuenta"], $array[$index]["Nit"]) : getMovimientosSaldoCuenta($fecha1,$fecha2,$array[$index]["Id_Plan_Cuenta"]);
        $debito = $saldos["Debito_$tipo_reporte"];
        $credito = $saldos["Credito_$tipo_reporte"];
        ...
        if ($tipo_nit === false) {
            ...
            /*if($array[$index]['Id_Plan_Cuenta'] == '329'){
            echo '<pre>';
            var_dump($saldos);
            echo '</pre>'; exit;
...
function armarDatosNit($cuentas, $fecha1= null, $fecha2= null) {
    foreach ($cuentas as $i => $cuenta) {
        $query = queryByNit($cuenta['Id_Plan_Cuenta'], false, $fecha1, $fecha2);
        ...
        if ($nits) {
            ...
            $query = queryByNit($cuenta['Id_Plan_Cuenta'], true,  $fecha1, $fecha2);
