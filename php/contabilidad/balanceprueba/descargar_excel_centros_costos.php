...
header('Content-Disposition: attachment;filename="Balance Prueba.xls"');
...
foreach ($centrosDeCostos as $ck => $centro_costo) {
    ...
    if ($tipo == 'General') {
        ...
        $objSheet->getStyle('A'.$j.':F'.$j)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $objSheet->getStyle('A'.$j.':F'.$j)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('00000000');
        $objSheet->getStyle('A'.$j.':F'.$j)->getFont()->setBold(true);
        $objSheet->getStyle('A'.$j.':F'.$j)->getFont()->getColor()->setARGB('FFFFFFFF');
        $objSheet->getStyle('A'.$j.':F'.$j)->getAlignment()->setWrapText(true);

        $objSheet->getStyle('A'.($j-1).':F'.($j-1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $objSheet->getStyle('A'.($j-1).':F'.($j-1))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('00000000');
        $objSheet->getStyle('A'.($j-1).':F'.($j-1))->getFont()->setBold(true);
        $objSheet->getStyle('A'.($j-1).':F'.($j-1))->getFont()->getColor()->setARGB('FFFFFFFF');
        $objSheet->getStyle('A'.($j-1).':F'.($j-1))->getAlignment()->setWrapText(true);
        ...
        $objSheet->getCell('A'.$j)->setValue("CUENTA");
        $objSheet->getCell('B'.$j)->setValue("NOMBRE CUENTA");
        $objSheet->getCell('C'.$j)->setValue("SALDO ANTERIOR");
        $objSheet->getCell('D'.$j)->setValue("DEBITOS");
        $objSheet->getCell('E'.$j)->setValue("CREDITOS");
        $objSheet->getCell('F'.$j)->setValue("NUEVO SALDO");

        $objSheet->getCell('A'.($j-1))->setValue("Centro de Costo:");
        $objSheet->getCell('B'.($j-1))->setValue( $centro_costo['Codigo'] . ' - ' . $centro_costo['Nombre'] );
        ...
        foreach ($balance as $i => $value) {
           ...
           /*  var_dump($centro_costo); */
            $saldo_anterior = obtenerSaldoAnterior($value['Naturaleza'], $balance, $i, $tipo_reporte, null, null, $movimientos, $centro_costo);
        ...
        foreach ($balance as $i => $value) {
            ...
            foreach ($nits as $z => $nit) {
                $saldo_anterior = obtenerSaldoAnterior($value['Naturaleza'], $nits, $z, $tipo_reporte, $nit['Nit'], $value['Id_Plan_Cuentas'],$movimientos, $centro_costo);
...
function obtenerSaldoAnterior($naturaleza, $array, $index, $tipo_reporte, $nit = null, $plan = null)
{
    ...
    //echo json_encode([$fecha1,$fecha2,null,null,$centro_costo]);exit;
    if ($fecha1 != '2019-01-01') {
        ...
        if ($nit === null) {
            ...
            $movimientos_lista = getMovimientosCuenta($fecha1, $fecha2, null, null, $centro_costo);
            ...
            $fecha2 = strtotime('-1 day', strtotime($fecha_ini)); // Un día antes de la fecha de inicio para sacar el corte de saldo final.
            $fecha2 = date('Y-m-d', $fecha2);
            $movimientos_lista = getMovimientosCuenta($fecha1, $fecha2, $nit, $plan, $centro_costo);
