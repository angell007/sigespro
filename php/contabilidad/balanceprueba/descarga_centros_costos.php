...
$query = "SELECT Id_Centro_Costo, Nombre, Codigo FROM Centro_Costo
WHERE Estado = 'Activo' AND Movimiento='Si'";
$oCon = new consulta();
$oCon->setQuery($query);
$oCon->setTipo('Multiple');

$centrosDeCostos = $oCon->getData();

//echo json_encode($centrosDeCostos);exit;
...
foreach ($centrosDeCostos as $ck => $centro_costo) {
    resetTotales($totales);
    //echo "<br>$ck - FOREACH CC ==================================================<br>";
    $movimientos = getMovimientosCuenta($fecha_ini, $fecha_fin, null, null, $centro_costo);

    // echo json_encode($movimientos);exit;
    $contenido .= '<table style="font-size:10px;margin-top:10px;" cellpadding="0" cellspacing="0">
    <tr>
        <td colspan="6" style="font-size:16px;margin-bottom:10px;" cellpadding="0" cellspacing="0" >' . $centro_costo['Id_Centro_Costo'] . ' - ' . $centro_costo['Codigo'] . ' - ' . $centro_costo['Nombre'] . '</td>
    </tr>
    <tr>
        ...
        if (true) {
           ...
           /* (SELECT IFNULL(SUM(BI.Debito_PCGA),0) FROM Balance_Inicial_Contabilidad BI WHERE BI.Fecha = '$ultimo_dia_mes' AND BI.Codigo_Cuenta LIKE CONCAT(PC.$column_1,'%') 
            ...
            Plan_Cuentas PC
             ...
             " . getStrCondiciones($centro_costo) . "
    ...
    //echo $query;exit;
            ...
            $exis2t=false;
               ...
               foreach ($balance as $i => $value) {
                   ...
                   if((float)($value['Debito_PCGA']) != 0 || (float)($value['Credito_PCGA']) != 0 || (float)($value['Debito_NIIF']) != 0 || (float)($value['Credito_NIIF']) != 0 ){
                       echo 'e';exit;
                   }
        ...
        exit; */
            ...
            foreach ($balance as $i => $value) {
                ...
                $saldo_anterior = obtenerSaldoAnterior($value['Naturaleza'], $balance, $i, $tipo_reporte, null, $movimientos, $centro_costo);
    ...
    ORDER BY PC.$column";
        ...
        exit; */
        foreach ($balance as $i => $value) {
            $balance[$i]['nits'] = nitsPorCuentasContables($value['Id_Plan_Cuentas'], $centro_costo);
        }
        foreach ($balance as $i => $value) {
            //echo "<br>$i - FOREACH BALANCE ==================================================<br>";
            $codigo = $tipo_reporte == 'Pcga' ? $value['Codigo'] : $value['Codigo_Niif'];
            $nombre_cta = $tipo_reporte == 'Pcga' ? $value['Nombre'] : $value['Nombre_Niif'];
            $saldo_anterior = obtenerSaldoAnterior($value['Naturaleza'], $balance, $i, $tipo_reporte, null, $movimientos, $centro_costo);
            //echo "<br>SALDO ANTERIOR ==================================================<br>";
            $debito = calcularDebito($codigo, $value['Tipo_P'], $movimientos);
            //echo "<br>DEBITO ==================================================<br>";
            $credito = calcularCredito($codigo, $value['Tipo_P'], $movimientos);
            //echo "<br>CREDITO ==================================================<br>";
            $nuevo_saldo = calcularNuevoSaldo($value['Naturaleza'], $saldo_anterior, $debito, $credito);
            //echo "<br>NUEVO SALDO ==================================================<br>";
            ...
            foreach ($nits as $j => $nit) {
                ...
                //echo "<br>$j - FOREACH NITS==================================================<br>";
                $saldo_anterior = obtenerSaldoAnterior($value['Naturaleza'], $nits, $j, $tipo_reporte, $nit['Nit'], $value['Id_Plan_Cuentas'], $centro_costo);
                $debito = $nit['Total_Debito_' . $tipo_reporte];
                $credito = $nit['Total_Credito_' . $tipo_reporte];
                $nuevo_saldo = calcularNuevoSaldo($value['Naturaleza'], $saldo_anterior, $debito, $credito);
                //echo "<br>NUEVO SALDO NIT ==================================================<br>";
    ...
    ORDER BY PC.$column";
        ...
        foreach ($balance as $i => $value) {
            $balance[$i]['tipos'] = getMovimientosPorTipo($fecha_ini, $fecha_fin, $value['Id_Plan_Cuentas'], $value['Movimiento'], $centro_costo);
        }
...
function getStrCondiciones($centro_costo)
{
    global $tipo_reporte;
...
obtenerSaldoAnterior($value['Naturaleza'], $balance, $i, $tipo_reporte, null, $movimientos, $centro_costo);
obtenerSaldoAnterior($value['Naturaleza'], $nits, $j, $tipo_reporte, $nit['Nit'], $value['Id_Plan_Cuentas'], $movimientos, $centro_costo);
function obtenerSaldoAnterior($naturaleza, $array, $index, $tipo_reporte, $nit = null, $plan = null, $cCosto)
{
    global $fecha_ini;
    //global $movimientos;
    ...
    //echo json_encode([$fecha1,$fecha2,null,null,$cCosto]);exit;
    if ($fecha1 != '2019-01-01') {
        ...
        if ($nit === null) {
            ...
            $movimientos_lista = getMovimientosCuenta($fecha1, $fecha2, null, null, $cCosto);
            ...
            $fecha2 = strtotime('-1 day', strtotime($fecha_ini)); // Un día antes de la fecha de inicio para sacar el corte de saldo final.
            $fecha2 = date('Y-m-d', $fecha2);
            $movimientos_lista = getMovimientosCuenta($fecha1, $fecha2, $nit, $plan, $cCosto);
...
function nitsPorCuentasContables($id_plan_cuentas, $centro_costo)
{
...
WHERE
    M.Id_Plan_Cuenta = $id_plan_cuentas AND M.Nit != 0  AND M.Estado != 'Anulado'
     " . ($centro_costo != false ? "AND M.Id_Centro_Costo = $centro_costo[Id_Centro_Costo]  AND PC.Codigo".($tipo_reporte == "Niif" ? "_Niif " : "") . $cond_exluir : "") . "
...
function getMovimientosCuenta($fecha1, $fecha2, $nit = null, $plan = null, $centro_costo)
{
    global $tipo_reporte, $cond_exluir;
    
    $rep=($tipo_reporte == "Niif" ? "_Niif" : "");
    $cc=$centro_costo["Id_Centro_Costo"];
    
    $tipo = $tipo_reporte != 'Pcga' ? '_Niif' : '';
    //echo "<br>GET MOV CUENTA==========================================================<br>"; 
    //var_dump($centro_costo);
    if ($nit === null) {
        ...
        BETWEEN '$fecha1' AND '$fecha2' AND MC.Estado != 'Anulado'
         " . ($centro_costo != false ? "AND MC.Id_Centro_Costo = $cc AND PC.Codigo$rep $cond_exluir" : "") . "
...
function getMovimientosPorTipo($fecha_ini, $fecha_fin, $id_plan_cuenta, $movimiento, $centro_costo)
{
    global $cond_exluir;
    if ($movimiento == 'S') {
        $query = "SELECT
        ...
        AND MC.Id_Plan_Cuenta = $id_plan_cuenta
         " . ($centro_costo != false ? "AND MC.Id_Centro_Costo = $centro_costo[Id_Centro_Costo]  AND PC.Codigo".($tipo_reporte == "Niif" ? "_Niif &nbps;" : "") . $cond_exluir :  "") . "
