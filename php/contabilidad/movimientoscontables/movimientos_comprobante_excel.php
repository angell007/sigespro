...
header('Content-Disposition: attachment;filename="Contabilizacion Recibo Caja.xls"');
...
WHERE
    MC.Estado = "Activo" AND Id_Modulo = '.$id_modulo.' AND Id_registro_Modulo ='.$id_registro.' ORDER BY Debe DESC';
...
FROM Movimiento_Contable MC
WHERE
    MC.Estado = "Activo" AND Id_Modulo = '.$id_modulo.' AND Id_registro_Modulo ='.$id_registro;
...
$contenido = '<table style="font-size:10px;margin-top:10px;" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td colspan="6" align="center" style="font-size:14px"><strong>Fecha '.fecha($datos['Fecha_Comprobante']).'</strong></td>
    </tr>
    <tr>
