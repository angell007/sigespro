...
$oItem = new complex('Comprobante','Id_Comprobante', $id_registro);
$datos = $oItem->getData();
unset($oItem);
...
/* HAGO UN SWITCH PARA TODOS LOS MODULOS QUE PUEDEN GENERAR PDF */
    ...
    $query = '
        ...
        Left JOIN Plan_Cuentas PC ON MC.Id_Plan_Cuenta = PC.Id_Plan_Cuentas
        WHERE   MC.Estado ="Activo" and
    ...
    // var_dump($movimientos_suma);
    // exit;

        // $query = 'SELECT (SELECT Nombre FROM Bodega WHERE Id_Bodega = I.Id_Bodega) AS Bodega, COUNT(DISTINCT(Id_Producto)) AS Cant_Producto, SUM(Cantidad) AS Cantidad, ROUND(SUM(Costo),2) AS Costo FROM Inventario I WHERE Id_Bodega != 0 AND Id_Punto_Dispensacion = 0 GROUP BY Id_Bodega';
        ...
        $codigos ='
            ...
            <h4 style="margin:5px 0 0 0;font-size:19px;line-height:22px;">'.$datos['Codigo'].'</h4>
            <h5 style="margin:5px 0 0 0;font-size:16px;line-height:16px;">Fecha '.fecha($datos['Fecha_Comprobante']).'</h5>
        ';
    ...
    if (count($movimientos) > 0) {
        ...
        foreach ($movimientos as $value) {
            ...
            $contenido .= '
            <tr>
            <td style="font-size:9px;width:78px;text-align:left;border:1px solid #cccccc;">
                '.$codigo.'
            </td>
            <td style="font-size:9px;width:150px;text-align:left;border:1px solid #cccccc;">
                '.$nombre_cuenta.'
            </td>
            <td style="font-size:9px;width:100px;text-align:right;border:1px solid #cccccc;">
                '.$value["Documento"].'
            </td>
            <td style="font-size:9px;width:100px;text-align:right;border:1px solid #cccccc;">
...
$cabecera='<table style="" >
              <tbody>
                <tr>
                  ...
                  <td class="td-header" style="width:430px;font-weight:thin;font-size:14px;line-height:20px;">
...
$pie='
    <table>
     <tr>
        <td style="font-size:11px;width:365px;vertical-align:middle;padding:2px 5px;text-align:center;">
        <br><br>______________________________<br>
            Elaborado Por<br>'.$funcionario["Nombre_Funcionario"].'<br>
            <small><em>'.date('d/m/Y H:i:s').'</em></small>
        </td>
     </tr>
    </table>';
