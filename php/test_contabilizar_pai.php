...
foreach($parciales as $par){
    ...
    $id_parcial = $par["Id_Nacionalizacion_Parcial"];
    
    echo $par['Codigo'] ."-". $id_parcial."<br>";
    ...
    EliminaMovimientosContables($parcial["Codigo"]);
    GuardarMovimientosContables($datos_movimiento_contable,$id_parcial);
}

function getParciales()
{
	global $queryObj;

	$query_productos =
		'SELECT Id_Nacionalizacion_Parcial, Codigo
		   FROM Nacionalizacion_Parcial
		   WHERE Fecha_Registro >= "2024-09-27"
		   #AND Id_Nacionalizacion_Parcial=137
		';

	$queryObj->SetQuery($query_productos);
	$parcial = $queryObj->ExecuteQuery('multiple');
...
function EliminaMovimientosContables($codigo)
{
	global $queryObj;

	$query = '
            DELETE FROM Movimiento_Contable
            WHERE
                Numero_Comprobante LIKE "%' . $codigo.'%"';
