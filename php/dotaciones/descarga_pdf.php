...
$query = 'SELECT D.*, CONCAT_WS(" ",F1.Nombres, F1.Apellidos) as Entrega, CONCAT_WS(" ",F2.Nombres, F2.Apellidos) as Recibe

FROM Dotacion D
INNER JOIN Funcionario F1 ON F1.Identificacion_Funcionario = D.Identificacion_Funcionario
INNER JOIN Funcionario F2 ON F2.Identificacion_Funcionario = D.Funcionario_Recibe
...
unset($oCon);
        ...
        $codigos ='
                      <h4 style="margin:5px 0 0 0;font-size:14px;line-height:14px;">Entrega Dotación</h4>
                      <h4 style="margin:5px 0 0 0;font-size:16px;line-height:16px;">ED000'.$funcionario["Id_Dotacion"].'</h4>
                      <h6 style="margin:5px 0 0 0;font-size:11px;line-height:11px;">'.fecha($funcionario["Fecha_Entrega"]).'</h6>
        ';
        $contenido = '<table style="border:1px solid #cccccc;"  cellpadding="0" cellspacing="0">
            ...
            <tr style="width:600px; " >
                            ...
                            <td colspan="3" style="width:600px;font-size:10px;text-align:left;border:1px solid #cccccc;padding:4px;padding-right:0;">'.$funcionario["Detalles_Entrega"].'</td>
            </tr>   
        </table>
        ...
        <table style="font-size:10px;margin-top:10px;" cellpadding="0" cellspacing="0">
         ...
         $conceptos = explode("|",$funcionario["Productos"]);
           ...
           $contenido.= '</table><br>
            ...
            <b style="font-size:10px;">Nota: El presente documento representa la entrega de los productos de Dotación y/o EPP antes mencionado, y su firma representa la aceptación del mismo</b>
