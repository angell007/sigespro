    ...
    if($value['Id_Producto_Inventario_Fisico']!=0){
        ...
        for ($i=0; $i < count( $id_inventario) ; $i++) { 
            if($i!=0){
                ...
                $oItem = new complex('Producto_Inventario_Fisico', 'Id_Producto_Inventario_Fisico', $id_inventario[$i]);
                $cantidad = number_format((INT)$value['Cantidad_Final'],0,'','');
                $conteo1 = number_format((INT)$value['Cantidad_Encontrada'],0,'','');
...
if($id_inventario){
    ...
    /*$datos_movimiento_contable['Id_Registro'] = $id_inventario;
