...
$resultado['mensaje'] = "Este modulo está en mantenimiento, estamos trabajando para mejorar el proceso.";
$resultado['tipo'] = "info";
$resultado['titulo'] = "Información!";
...
$cod = generarConsecutivo('Ingreso',$mes);
$datos['Codigo']=$cod;
...
if(count($facturas)>0) {
    ...
    foreach ($facturas as $fact) {
        ...
        $oItem->Excenta = $fact['Exenta'];
        $oItem->Gravada = $fact['Gravada'];
        $oItem->Iva = $fact['Iva'];
        $oItem->Total = $fact['Total_Compra'];
        $oItem->Neto_Factura = $fact['Neto_Factura'];
        $valor = number_format((FLOAT)$fact['ValorIngresado'],2,".","");
        $oItem->Valor = $valor;
        $oItem->Id_Factura = $fact['Id_Factura'];
        $oItem->ValorDescuento = isset($fact['ValorDescuento']) ? $fact['ValorDescuento'] : 0;
        $oItem->ValorMayorPagar = isset($fact['ValorMayorPagar']) ? $fact['ValorMayorPagar'] : 0;
