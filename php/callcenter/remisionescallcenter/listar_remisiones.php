...
$condicion = 'WHERE Estado IN ("Enviada","Recibida") AND Tipo_Destino = "Punto_Dispensacion" AND Observaciones LIKE "%CALLCENTER%" ';
...
$query = 'SELECT R.Id_Remision,  
IFNULL(ROUND((RCM.Actuales*100/RCM.Total_Llamadas),0),0) AS Porcentaje, 
IFNULL((SELECT "Si" AS Tipo FROM Servicio_Punto_Dispensacion SPD WHERE SPD.Id_Punto_Dispensacion = R.Id_Destino AND SPD.Id_Servicio = 2 LIMIT 1),"No") AS NoPos
FROM Remision R 
LEFT JOIN (SELECT RCC.Id_Remision, COUNT(RCC.Id_Remision_Callcenter) as Actuales, MAX(RCC.Total_Llamadas) AS Total_Llamadas FROM Remision_Callcenter RCC WHERE Id_Remision_Callcenter_Anterior IS NULL GROUP BY RCC.Id_Remision ) RCM ON RCM.Id_Remision = R.Id_Remision
' . $condicion.' HAVING  (NoPos = "Si" AND Porcentaje != 100 ) ';
...
$query = 'SELECT R.*, IFNULL(ROUND((RCM.Actuales*100/RCM.Total_Llamadas),0),0) AS Porcentaje, IFNULL((SELECT "Si" AS Tipo FROM Servicio_Punto_Dispensacion SPD WHERE SPD.Id_Punto_Dispensacion = R.Id_Destino AND SPD.Id_Servicio = 2 LIMIT 1),"No") AS NoPos, (CASE   
WHEN R.Estado_Alistamiento = 0 THEN "1"   
WHEN R.Estado_Alistamiento = 1 THEN "2"
WHEN R.Estado_Alistamiento = 2 THEN "Listo"
END ) as Fase, DATE_FORMAT(Fecha, "%d/%m/%Y") AS Fecha_Remision, (SELECT COUNT(*) FROM Producto_Remision PR WHERE PR.Id_Remision = R.Id_Remision) as Items 
FROM Remision R 
LEFT JOIN (SELECT RCC.Id_Remision, COUNT(RCC.Id_Remision_Callcenter) as Actuales, MAX(RCC.Total_Llamadas) AS Total_Llamadas FROM Remision_Callcenter RCC WHERE Id_Remision_Callcenter_Anterior IS NULL GROUP BY RCC.Id_Remision ) RCM ON RCM.Id_Remision = R.Id_Remision
'.$condicion.' HAVING  (NoPos = "Si" AND Porcentaje != 100 ) ORDER BY Codigo DESC, Fecha DESC LIMIT '.$limit.','.$tamPag;
