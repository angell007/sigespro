...
foreach($productos as $dir){ $i++;
    ...
    $query="SELECT D.Id_Dispensacion, D.Numero_Documento ,SUM(PD.Cantidad_Formulada) AS Formulada, SUM(PD.Cantidad_Entregada) AS Entregada, SUM(PF.Cantidad) AS Cantidad_Factura, PF.Precio, PF.Impuesto,
           ...
           WHERE F.Tipo='Factura' AND F.Estado_Factura != 'Anulada' 
