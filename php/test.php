...
$id_nuevo = $_REQUEST['id_nuevo'] ? $_REQUEST['id_nuevo'] : '';
$cumNuevo = $_REQUEST['cum1'] ? $_REQUEST['cum1'] : '';
$cambio = $_REQUEST['cambio'] ? $_REQUEST['cambio']: null;
...
if($query_update && $query_update != ';' &&  $query_update != ';;') {
    header("content-type: application/json");
    echo $query_update; exit;
}

$contenido = "<div class='row'  style='display: flex;flex-direction: row;flex-wrap: wrap;'>
...
function getQuery($ids)
{
    ...
    foreach ($tablas as $tabla) {
        ...
        // echo json_encode(array($productos)); exit;
        if( ($id_nuevo && $tabla!='Producto' && in_array($id_nuevo, explode(",",str_replace(" ", "", $productos)))) || ($cambio && $id_nuevo )){
            $query1 = "UPDATE $tabla SET Id_Producto = $id_nuevo Where Id_Producto in($productos) \n ";
            array_push($sqlupdate, $query1);
        }
    ...
    if($query1){
        // $productos = explode(",",str_replace(" ", "", $productos)); 
        // echo json_encode($productos[$id_nuevo]); exit;
        // $productos = array_splice($productos,$id_nuevo, 1);
        // $productos = implode(",", $productos);
        array_push($sqlupdate, "DELETE FROM Producto WHERE Id_Producto in($productos) and Id_Producto != $id_nuevo");
        $query_update .= implode(";", $sqlupdate).";";
    }
...
{
    global $cumNuevo, $query_update, $cambio;
    $tablas = "Producto_Agotado,Bodega_Inicial,Lista_Precio_Proveedor,Precio_Regulado,Producto_Cohorte_Temp,Producto_Evento,Producto_Evento_Temporal,Producto_Lista_Ganancia,Producto_NoPos,Producto_No_Encontrados,Temporal_Factura_Rem,Z_Cum_Excluidos,Z_Factura_Vieja,Z_Mipres_Reportar";
    $tablas = explode(",", $tablas);
    $productos = $cums;
    $sql = [];
    $sqlupdate = [];
    foreach ($tablas as $tabla) {
        ...
        // echo json_encode(explode(",",str_replace("'","", $productos)) ). $cumNuevo; exit;
        if($cumNuevo && in_array($cumNuevo, explode(",",str_replace("'","", $productos)))  || ($cambio && $cumNuevo )){
            $query1 = "UPDATE $tabla T set T.$col_Cum = '$cumNuevo' where T.$col_Cum in($productos)\n ";
            array_push($sqlupdate, $query1);
        }
    ...
    if($cumNuevo && $query1){
        $query1 = "UPDATE Producto T set T.Codigo_Cum = '$cumNuevo' where T.Codigo_Cum in($productos)\n ";
        array_push($sqlupdate, $query1);
    }
    if($query1){
        $query_update .= implode(";", $sqlupdate).";";    
