<?php
	...
	$query_rem='SELECT Id_Remision, Nombre_Destino, Id_Destino, Codigo FROM Remision  WHERE Id_Remision IN ('.$rem.')';
	$oCon= new consulta();
	...
	foreach ($remisiones as  $value) {
		if ($l == 0) {
			$punto_dispensacion = $value['Nombre_Destino'];
		}
...
if(count($queryInsert)>0){
	...
	foreach ($modelo as  $item) {
		...
		if(ValidarCantidad($inventario['Cantidad_Pendientes'],$item['Cantidad'])){
			...
			if ($comparar["Cantidad_Entregada"] == 0) { // Cambiando el producto por el del otro laboratorio
                ...
                $oItem = new complex('Producto_Dispensacion',"Id_Producto_Dispensacion",$item["Id_Producto_Dispensacion"]); // Modificando los datos del producto para poder registrar un producto con el otro laboratorio y así cumplir entregar el pendiente completo.
			...
			$oItem->Entregar_Faltante=$item["Entregar_Faltante"];
			$oItem->save();
			unset($oItem);
			...
			//REGISTRAR LOS MENSAJES EN UN ARRAY PARA ENVIARLOS DESPUES DE DESCARGAR LOS PENDIENTES
			//EnviarMensaje($item['Id_Producto'], $item['Id_Paciente'], $func);
			CargarMensajes($item['Id_Producto'], $item['Id_Paciente'], $func, $item['Id_Remision'], $item['Id_Dispensacion']);
	...
	//ENVIAR TODOS LOS MENSAJES REGISTRADOS DURRANTE LA DESCARGA DE PENDIENTES
	EnviarMensajes();
