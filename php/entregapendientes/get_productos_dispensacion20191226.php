...
foreach ($productos_remision as $key => $value) {
    ...
    WHERE D.Id_Tipo_Servicio!=7 AND (PD.Cantidad_Formulada>PD.Cantidad_Entregada) AND PD.Id_Producto='.$value['Id_Producto'].$condicion.' AND D.Estado_Dispensacion!="Anulada" AND PA.Nit!=830074184  ORDER BY PD.Id_Producto_Dispensacion ASC ';
    ...
    foreach ($productos_dispensacion as $i => $item) {$k++;
        if($cantidad>0){
            if((INT)$item['Cantidad']<=$cantidad){
                ...
                $productos_dispensacion[$k]['Cantidad']=$cantidad;
                $resultado[$j]=$productos_dispensacion[$k];
                $temporal['Id_Producto_Dispensacion']=$item['Id_Producto_Dispensacion'];
                $temporal['Id_Remision']=$item['Id_Remision'];
                $texto[$j]=(object)$temporal;
                $cantidad=0;
                $j++;
                //unset($productos_dispensacion[$i]);
            }
