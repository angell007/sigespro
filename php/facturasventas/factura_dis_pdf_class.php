...
require_once(__DIR__.'/../../class/html2pdf.class.php');
include_once(__DIR__.'/../../class/NumeroALetra.php');
...
class FacturaDis
{
    ...
    public function __construct($id, $ruta)
    {
        $this->id = $id;
        $this->ruta = $ruta;
        // $this->MY_FILE= $_SERVER["DOCUMENT_ROOT"] ."/";	
    }
    ...
    public function GenerarPdf()
    {
        ...
        if ($this->tipo && $this->tipo == 'Homologo') {
            ...
            INNER JOIN Cliente C
            ON FV.Id_Cliente = C.Id_Cliente
            WHERE FV.Id_Factura_Asociada = ' . $this->id;
        ...
        $cabecera .= '</td>
                ...
                <tr>
                     ...
                     Por favor POR FAVOR ABSTENERSE PRACTICAR RETENCIÓN EN LA FUENTE por ICA, <br> 
                     SOMOS GRANDES CONTRIBUYENTES DE ICA EN BUCARAMANGA en la ciudad de Bucaramanga. Resolución 3831 de 18/04/2022
                     </td>
        ...
        if ($func['Firma'] != '' && file_exists($MY_FILE . "DOCUMENTOS/" . $func["Identificacion_Funcionario"] . "/" . $func['Firma'] )) {
        ...
        $pie .= '<tr>
            <td style="font-size:7px;width:778px;background:#f3f3f3;vertical-align:middle;padding:1px 5px;height:10px;">
                ...
                Auorizaciòn de Facturacion # ' . $fact["Resolucion"] . ' Desde ' . $this->fecha($fact["Fecha_Inicio"]) . ' Hasta ' . $this->fecha($fact["Fecha_Fin"]) . '
        ...
        $contenido = '<table  cellspacing="0" cellpadding="0" >
	        	    <tr>
	        		<td style="' . $width_prod . 'font-size:8px;background:#c6c6c6;text-align:center;">Descripciónnn</td>
    ...
    '; */
        ...
        $content = '<page backtop="230px" backbottom="140px" ' . $marca_agua . '>
        ...
        try {
            ...
            if ($this->ruta == '') {
                ...
                $html2pdf->Output($_SERVER['DOCUMENT_ROOT'].$direc, "F");
            }
        } catch (\Throwable $e) {
