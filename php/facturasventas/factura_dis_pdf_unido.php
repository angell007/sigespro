...
$query = 'SELECT 
            D.Id_Servicio,FV.Cufe, FV.Id_Factura, FV.Id_Factura_Asociada, FV.Codigo_Qr, FV.Fecha_Documento as Fecha , FV.Observacion_Factura as observacion, FV.Codigo as Codigo,IF(FV.Condicion_Pago=1,"CONTADO",FV.Condicion_Pago) as Condicion_Pago , FV.Fecha_Pago as Fecha_Pago , FV.Tipo as tipo,
...
$query = 'SELECT 
        ...
        P.Id_Producto, 
        P.Codigo_Cum as Cum, 
        IFNULL(PFV.Fecha_Vencimiento,0) as Vencimiento, 
...
$titulo = $cliente["tipo"]=='Homologo' ? 'Factura de Venta (HOMOLOGO)' : 'Factura de Venta ' . ($cliente['tipo'] == 'Homologo' ? '(HOMOLOGO)' : '');

if ($cliente["tipo"] == 'Homologo') {
    $condicion_consulta = 'WHERE FV.Id_Factura = '.$cliente["Id_Factura_Asociada"];
}else{
   $condicion_consulta = 'WHERE FV.Id_Factura_Asociada = '.$cliente["Id_Factura"]; 
}

$query = 'SELECT 
            D.Id_Servicio, FV.Cufe, FV.Id_Factura, FV.Codigo_Qr, FV.Fecha_Documento as Fecha , FV.Observacion_Factura as observacion, FV.Codigo as Codigo,IF(FV.Condicion_Pago=1,"CONTADO",FV.Condicion_Pago) as Condicion_Pago , FV.Fecha_Pago as Fecha_Pago , FV.Tipo as tipo,
            C.Id_Cliente as IdCliente ,C.Nombre as NombreCliente, C.Direccion as DireccionCliente, (SELECT Nombre FROM Municipio WHERE Id_Municipio = C.Ciudad) as CiudadCliente, C.Credito as CreditoCliente, C.Cupo as CupoCliente, (SELECT CONCAT_WS(" ",Primer_Nombre,Segundo_Nombre,Primer_Apellido,Segundo_Apellido, CONCAT("- ", UPPER(IF(Id_Regimen=1,"Contributivo","Subsidiado")))) FROM Paciente WHERE Id_Paciente=D.Numero_Documento) AS Nombre_Paciente, D.Numero_Documento, FV.Cuota, D.Codigo AS Cod_Dis, (SELECT Nombre FROM Tipo_Servicio WHERE Id_Tipo_Servicio = D.Id_Tipo_Servicio) AS Tipo_Servicio, C.Tipo_Valor,(SELECT  P.Eps FROM Dispensacion D INNER JOIN (SELECT EPS, Id_Paciente FROM Paciente ) P ON D.Numero_Documento=P.Id_Paciente   WHERE Id_Dispensacion=FV.Id_Dispensacion) as Eps
          FROM Factura FV
          INNER JOIN Dispensacion D ON FV.Id_Dispensacion = D.Id_Dispensacion 
          INNER JOIN Cliente C
...
$oCon= new consulta();
$oCon->setQuery($query);
$cliente2 = $oCon->getData();
unset($oCon);

if ($cliente["tipo"] == 'Homologo') {
    $cliente['Cuota'] = 0;
    $cliente['observacion'] = "<br><strong>ESTA ES UNA FACTURA HOMOLOGO POS, QUE CORRESPONDE AL DESCUENTO APLICADO A LA FACTURA - ".$cliente2["Codigo"]."</strong>";
}   
if($cliente2){
    $query = 'SELECT 
            CONCAT_WS(" ",P.Nombre_Comercial, P.Presentacion, P.Concentracion, " (", P.Principio_Activo,") ", P.Cantidad," ", P.Unidad_Medida ) as producto, 
           ...
           ON PD.Id_Producto_Dispensacion = PFV.Id_Producto_Dispensacion 
           INNER JOIN Producto P 
           ON P.Id_Producto = PFV.Id_Producto  WHERE PFV.Id_Factura =  '.$cliente2["Id_Factura"] ;
    $oCon= new consulta();
    $oCon->setQuery($query);
    $oCon->setTipo("Multiple");
    $productos2 = $oCon->getData();
    unset($oCon); 
    
    $query5 = 'SELECT SUM(Subtotal) as TotalFac FROM Producto_Factura WHERE Id_Factura = '.$cliente2["Id_Factura"] ;

    $oCon= new consulta();
    $oCon->setQuery($query5);
    $totalFactura2 = $oCon->getData();
    unset($oCon);
    
    $titulo2 = $cliente2["tipo"]=='Homologo' ? 'Factura de Venta (HOMOLOGO)' : 'Factura de Venta ' . ($cliente2['tipo'] == 'Homologo' ? '(HOMOLOGO)' : '');

    if ($cliente2["tipo"] == 'Homologo') {
        $cliente2['Cuota'] = 0;
        $cliente2['observacion'] = "<br><strong>ESTA ES UNA FACTURA HOMOLOGO POS, QUE CORRESPONDE AL DESCUENTO APLICADO A LA FACTURA - ".$cliente["Codigo"]."</strong>";
    }
    
    if ($cliente2["tipo"]!="Homologo") {
        $col_desc2 = '<td style="font-size:7px;line-height:8px;background:#c6c6c6;text-align:center;">Descuento</td>';
        $width_prod3 = 'width:392px;';
        $width_prod4 = 'width:225px;';
    }else{
        $col_desc2 = '';
        $width_prod3 = 'width:450px;';
        $width_prod4 = 'width:285px;';
    }
}else{
  $segunda=false;
}    
...
if($segunda===true){
    $codigos2 ='
    <span style="margin:-5px 0 0 0;font-size:13px;line-height:13px;">'.$titulo2.'</span>
    <h3 style="margin:0 0 0 0;font-size:15px;line-height:15px;">'.$cliente2["Codigo"].'</h3>
    <h5 style="margin:5px 0 0 0;font-size:8px;line-height:8px;">F. Expe.:'.fecha($cliente2["Fecha"]).'</h5>
    <h4 style="margin:5px 0 0 0;font-size:8px;line-height:8px;">F. Venc.:'.fecha($cliente2["Fecha_Pago"]).'</h4>
    <h4 style="margin:0 0 0 0;font-size:13px;line-height:13px">'.$cliente2["Cod_Dis"].'</h4>
    <h4 style="margin:0 0 0 0;font-size:13px;line-height:13px">'.$cliente2["Tipo_Servicio"].'</h4>
    ';
    $condicion_pago2 = $cliente2["Condicion_Pago"] == "CONTADO" ? $cliente2["Condicion_Pago"] : $cliente2["Condicion_Pago"] . " Días";
    $nombre_paciente2 = utf8_decode(explode('-',$cliente2["Nombre_Paciente"])[0]);
    $regimen2 = explode('-',$cliente2["Nombre_Paciente"])[1];
   ...
   $cabecera2='<table style="" >
              <tbody>
                <tr>
                  ...
                  if($fact["Tipo_Resolucion"]=="Resolucion_Electronica"){
                    $cabecera2.='<img src="'.($cliente2["Codigo_Qr"] =='' ? $_SERVER["DOCUMENT_ROOT"].'assets/images/sinqr.png' : $_SERVER["DOCUMENT_ROOT"].'ARCHIVOS/FACTURACION_ELECTRONICA/'.$cliente2["Codigo_Qr"] ).'" style="max-width:100%;margin-top:-8px;" />';
                  }else{
                    $cabecera2.='<img src="'.($cliente2["Codigo_Qr"] =='' ? $_SERVER["DOCUMENT_ROOT"].'assets/images/sinqr.png' : $_SERVER["DOCUMENT_ROOT"].'IMAGENES/QR/'.$cliente2["Codigo_Qr"] ).'" style="max-width:100%;margin-top:-8px;" />';
                  }
                  $cabecera2.='</td>
                </tr>
                <tr>
                    <td colspan="3" style="font-size:9px"> <br>
            ...
            <table cellspacing="0" cellpadding="0" style="text-transform:uppercase;margin-top:8px;">
                <tr>
                    ...
                    '.number_format($cliente2["IdCliente"],0,",",".").'
                    </td>
                </tr>
                <tr>
                    <td style="font-size:8px;width:60px;vertical-align:middle;padding:2px;">
                    ...
                    <td style="font-size:8px;width:510px;vertical-align:middle;padding:2px;">
                    '.trim($cliente2["DireccionCliente"]).'
                    </td>
                    <td style="font-size:8px;width:85px;vertical-align:middle;padding:2px;">
                ...
                <tr>
                    ...
                    <td style="font-size:8px;width:510px;vertical-align:middle;padding:2px;">
                    '.trim($nombre_paciente2) . ' - <strong>' .$regimen2.'</strong>
                    </td>
                    <td style="font-size:8px;width:85px;vertical-align:middle;padding:2px;">
                    <strong>Nº Documento:</strong>
                    </td>
                    <td style="font-size:8px;width:110px;vertical-align:middle;padding:2px;">
                    '.$cliente2["Numero_Documento"].'
                    </td>
                </tr>
            </table>
...
$contenido2 = '<table  cellspacing="0" cellpadding="0" >
	        	    <tr>
	        		<td style="'.$width_prod3.'font-size:8px;background:#c6c6c6;text-align:center;">Descripción</td>
                ...
                $subtotal_acum = 0;
                    ...
                    if ($cliente2['Tipo_Valor'] == 'Cerrada') {
                        $decimales = 0;
                    }
	        	    ...
	        	    foreach($productos2 as $prod){
	        	    	...
	        	    	$contenido2.='<tr>
	        		<td style="'.$width_prod3.'padding:2px 3px;font-size:7px;text-align:left;border:1px solid #c6c6c6;vertical-align:middle;line-height:9px;height:auto;">
                    ...
                    if ($cliente2["tipo"]!="Homologo") {
                        ...
                        if ($clientes["IdCliente"] == 890500890) { // SI ES NORTE DE SANTANDER
                            $decimales_dcto = 0;
                        }
                        ...
                        $contenido2 .= '<td style="padding:2px 3px;font-size:7px;;text-align:center;border:1px solid #c6c6c6;vertical-align:middle;width:50px;line-height:9px;height:auto;">
	        		...
	        		$contenido2 .= '<td style="padding:2px 3px;font-size:7px;text-align:right;border:1px solid #c6c6c6;vertical-align:middle;width:60px;line-height:9px;height:auto;">
                    ...
                    if ($cliente2["IdCliente"] == 890500890) { // SI ES NORTE DE SANTANDER
	             ...
	             <table style="margin-top:8px;margin-bottom:0;" >
	             	...
	             	<tr>
	             	   <td style="padding:2px 3px;font-size:7px;border:1px solid #c6c6c6;width:446px;line-height:8px;">
	             	   	...
	             	   	'.$cliente2["observacion"].'
	             	   </td>
	             	   <td style="padding:2px 3px;font-size:7px;border:1px solid #c6c6c6;width:90px;line-height:8px;"></td>
	             	</tr>
                 </table>';
                 
                 if ($cliente2["Id_Servicio"] = '2' && $cliente2['tipo']!="Homologo") {
                    $marca_agua2 = 'backimg="'.$_SERVER["DOCUMENT_ROOT"].'assets/images/SelloNoPos.png" backimgw="50%"';
                }
...
$cabecera='<table style="" >
              <tbody>
                <tr>
                  ...
                  <td style="width:100px;">';
                  if($fact["Tipo_Resolucion"]=="Resolucion_Electronica"){
                    $cabecera.='<img src="'.($cliente["Codigo_Qr"] =='' ? $_SERVER["DOCUMENT_ROOT"].'assets/images/sinqr.png' : $_SERVER["DOCUMENT_ROOT"].'ARCHIVOS/FACTURACION_ELECTRONICA/'.$cliente["Codigo_Qr"] ).'" style="max-width:100%;margin-top:-8px;" />';
                  }else{
                    $cabecera.='<img src="'.($cliente["Codigo_Qr"] =='' ? $_SERVER["DOCUMENT_ROOT"].'assets/images/sinqr.png' : $_SERVER["DOCUMENT_ROOT"].'IMAGENES/QR/'.$cliente["Codigo_Qr"] ).'" style="max-width:100%;margin-top:-8px;" />';
                  }
                  $cabecera.='</td>
                </tr>
                <tr>
...
$pie='<table cellspacing="0" cellpadding="0" style="text-transform:uppercase;margin-top:4px;">';
	...
	$pie.='<tr>
		<td style="font-size:7px;width:778px;background:#f3f3f3;vertical-align:middle;padding:1px 5px;height:10px;">
			...
			Autorizacion de Facturacion # '.$fact["Resolucion"].' Desde '.fecha($fact["Fecha_Inicio"]).' Hasta '.fecha($fact["Fecha_Fin"]).' Habilita Del No. '.$fact["Codigo"].$fact["Numero_Inicial"].' Al No. '.$fact["Codigo"].$fact["Numero_Final"].' Actividad economica principal 4645<br>
		</td>
	</tr>
	<tr>
...
if ($cliente["Id_Servicio"] == '2' && $cliente['tipo']!="Homologo") {
...
$content = '<page backtop="215px" backbottom="105px" '.$marca_agua.'>
		<page_header>'.
                    $cabecera.
		'</page_header>
		<page_footer>'.$pie.'</page_footer>
            ...
            ';
if($segunda==true){
    $content .='<page backtop="215px" backbottom="105px" '.$marca_agua2.'>
		<page_header>'.
                    $cabecera2.
		'</page_header>
		<page_footer>'.$pie.'</page_footer>
