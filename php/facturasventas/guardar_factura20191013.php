...
/* $resultado['titulo'] = "Mantenimiento!";
$resultado['mensaje'] = "Se está realizando mantenimiento en el modulo de facturación. Disculpe las molestias.";
$resultado['tipo'] = "info";
...
if(count($encabezadoFactura) >0){
    if($fact=='true'){
      ...
      if(!$factura){
        ...
        $resultado['mensaje'] = "Actualmente no tiene un resolucion activa por favor revise  ";
...
function guardarFactura($datos, $productos, $tipo,$mod){
   ...
   $datos['Id_Funcionario'] = $funcionario; */
    ...
    foreach($productos as $producto){
        ...
        $oItem->Id_Producto_Dispensacion=$producto['Id_Producto_Dispensacion'] !='' ? $producto['Id_Producto_Dispensacion'] : '0';
        $impuesto = $producto['Impuesto'] != 0 ? (FLOAT) $producto['Impuesto'] * 100 : 0;
        if (!aplicaIva($datos['Id_Cliente']) && $impuesto != 0) {
            $oItem->Impuesto = '0';    
        } else {
    ...
    $datos_movimiento_contable['Id_Regimen'] = $datos['Id_Regimen'];
