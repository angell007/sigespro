...
$query = 'SELECT AR.*, (SELECT GROUP_CONCAT(F.Factura SEPARATOR " / ") FROM Factura_Acta_Recepcion F
 WHERE F.Id_Acta_Recepcion=AR.Id_Acta_Recepcion GROUP BY F.Id_Acta_Recepcion) AS Factura, B.Nombre as Nombre_Bodega,
  (SELECT P.Nombre FROM Proveedor P WHERE P.Id_Proveedor=AR.Id_Proveedor) AS Proveedor,
   (SELECT PAR.Codigo_Compra FROM Producto_Acta_Recepcion PAR WHERE 
   PAR.Id_Acta_Recepcion=AR.Id_Acta_Recepcion GROUP BY AR.Id_Acta_Recepcion) AS Codigo_Compra
FROM Acta_Recepcion AR
INNER JOIN Bodega_Nuevo B
ON AR.Id_Bodega_Nuevo=B.Id_Bodega_Nuevo
...
$query = "SELECT F.Id_Factura_Acta_Recepcion as Id_Factura,F.Factura, F.Fecha_Factura, F.Archivo_Factura
...
,CONCAT(PRD.Nombre_Comercial," ",PRD.Laboratorio_Comercial)),  "\n Invima:", PRD.Invima, " CUM:", PRD.Codigo_Cum  ) as Nombre_Producto,
...
$resultado["No_Conforme"]=getNoConforme($id_acta); 
...
function getNoConforme($id_acta)
{
    $query = "SELECT PNC.*, 
    
                    PRD.Nombre_Comercial, PRD.Laboratorio_Comercial, PRD.Laboratorio_Generico, 
                    CONCAT( IFNULL(CONCAT(PRD.Principio_Activo,' ',PRD.Presentacion,' ',PRD.Concentracion,' ', PRD.Cantidad,' ', PRD.Unidad_Medida)
                    ,CONCAT(PRD.Nombre_Comercial,' ',PRD.Laboratorio_Comercial)),' CUM:',  PRD.Codigo_Cum) as Nombre_Producto, 
                ...
                AND PNC.Id_Acta_Recepcion = $id_acta";
    $oCon= new consulta();
    $oCon->setTipo('Multiple');
    $oCon->setQuery($query);
