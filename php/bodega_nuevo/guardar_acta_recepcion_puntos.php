...
switch($tipoCompra){
    ...
    case "Nacional":{
        $query ="SELECT	Id_Orden_Compra_Nacional as id,Codigo,Identificacion_Funcionario,Id_Bodega,Id_Proveedor
    ...
    if ($prod['No_Conforme'] == 2) {
        ...
        foreach($prod["producto"] as $item){$i++;
           ...
           // $precio = number_format($prod['Precio'],2,".","");            
            $subtotal = number_format((INT)$subtotal,0,".","");
        ...
        /*
        ! SE QUITA QUE AFECTE LAS LISTAS DE GANANCIA POR ORDEN DEL INGENIERO FREDDY, EN CASO QUE SE REQUIERA ACTIVAR, DESCOMENTAR ESTE BLOQUE
