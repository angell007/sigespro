<?php
	...
	function GetCampos($idTipoServicio, $modulo){
		...
		$condicion='';
		if($modulo!='Ambos'){
			$condicion="  AND (Modulo = '".$modulo."' OR Modulo = 'Ambos')";
		}

		$query='SELECT *, Nombre AS Field_Name FROM Campos_Tipo_Servicio WHERE Id_Tipo_Servicio = '.$idTipoServicio.' AND Tipo_Campo = "Cabecera" AND Estado="Activo" '.$condicion;
