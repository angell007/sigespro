...
include_once('../../class/class.facturaccionmasiva_dev.php');
...
echo json_encode($response);
 ...
 function cantidadInventario($id_inventario_nuevo) {
    /** Modifcado el 20 de Julio 2020 Augusto - Cambia Inventario por Inventario_Nuevo */
    $query = "SELECT (Cantidad-Cantidad_Apartada-Cantidad_Seleccionada) AS Cantidad FROM Inventario_Nuevo WHERE Id_Inventario_Nuevo = $id_inventario";
    ...
    if(strtolower( $dispensacion['Tipo_Servicio'])=='evento' &&  $dispensacion['Id_Servicio']=='1' ){
        ...
        if(count($pendientes)==0){
            ...
            if(count($productos_sin_precio)==0){
                ...
                $facturaccion->Facturacion($idDis,12345,$tipo);
                ...
                echo "Existen productos con precio cero (0).";
                exit;
            }
        } else {
            echo "El producto tenía pendientes";
            exit;
        }      
    }
