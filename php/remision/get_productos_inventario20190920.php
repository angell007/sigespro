<?php
	...
	function SetCondiciones($req){
       ...
       if($tipo_bodega[0]=='Bodega'){
            if($tipo_bodega[1]=='Bodega' && ($bodega =='6' || $bodega =='8' || $bodega =='9')  ){
                $condicion_principal=" WHERE Id_Bodega=$req[id_origen] ";
            }else{
    ...
    function GetQuery($tipo){
        ...
        if($tipo=='Interna'){
            $query='SELECT (SELECT Nombre FROM Categoria WHERE Id_Categoria=PRD.Id_Categoria) as Categoria,PRD.Id_Categoria,  PRD.Id_Producto,ROUND(IFNULL(AVG(I.Costo),0)) as Precio, PRD.Embalaje,SUM(I.Cantidad-(I.Cantidad_Apartada+I.Cantidad_Seleccionada)) as Cantidad_Disponible, CONCAT(PRD.Principio_Activo," ",PRD.Presentacion," ",PRD.Concentracion," ", PRD.Cantidad," ", PRD.Unidad_Medida) as Nombre,  PRD.Nombre_Comercial, PRD.Laboratorio_Comercial,PRD.Laboratorio_Generico,PRD.Codigo_Cum, PRD.Cantidad_Presentacion, 0 as Seleccionado, NULL as Cantidad, (
              ...
              END
            ) as Impuesto, (SELECT ROUND(AVG(Costo)) FROM Inventario WHERE Id_Bodega!=0 AND Id_Producto=I.Id_Producto) as Costo
            FROM Inventario I
            ...
            $query='SELECT (SELECT Nombre FROM Categoria WHERE Id_Categoria=PRD.Id_Categoria) as Categoria,PRD.Id_Categoria,  PRD.Id_Producto,
          ...
          END
            ...
            ON PRD.Codigo_Cum = PRG.Codigo_Cum
            '.$condicion_principal.$condicion.' AND LG.Id_Lista_Ganancia ='.$datoscliente['Id_Lista_Ganancia'].$having ;
        }
    ...
    function GetCosto($id,$costo){
        ...
        FROM
        (
        SELECT Precio FROM Producto_Acta_Recepcion PR  WHERE PR.Id_Producto=$id ORDER BY PR.Id_Producto_Acta_Recepcion DESC LIMIT 3 
        ) r 
        ";
