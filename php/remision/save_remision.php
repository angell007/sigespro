...
{
     $datos["FIni_Rotativo"]=date("Y-m-d",strtotime($rotativo["Fecha_Inicio"]));
     $datos["FFin_Rotativo"]=date("Y-m-d",strtotime($rotativo["Fecha_Fin"]));
...
function GetIdCategoria($tipo){
     ...
     global $queryObj;
     $query = "SELECT Id_Categoria FROM Categoria WHERE Nombre LIKE '%$tipo%' ";  
