...
$nuevafecha= date('Y-m-d', $nuevafecha);
    ...
    INNER JOIN Inventario I ON PPR.Id_Producto=I.Id_Producto
    	...
    	WHERE I.Id_Bodega='.$bodega.' 
    	AND (I.Cantidad-I.Cantidad_Apartada-I.Cantidad_Seleccionada)>0 /*AND (MOD((I.Cantidad),PRD.Cantidad_Presentacion)=0)*/ AND I.Fecha_Vencimiento>"'.$nuevafecha.'" AND PPR.Id_Punto_Dispensacion='.$id_punto.' 
	...
	foreach($productos as $producto){ $i++;
		...
		if ($producto['Id_Producto']!=$idproducto){
			...
			if($producto["Nombre"]==''){
				...
				$resultado[$pos]["producto"]=$producto["Nombre_Comercial"]." EMB- ".$producto["Embalaje"]." LAB- ".$producto["Laboratorio_Comercial"];
			}else{
				$resultado[$pos]["Nombre"]=$producto["Nombre"];
				$resultado[$pos]["producto"]=$producto["Nombre"];
			}
			...
			$resultado[$pos]["Pendientes"]=$producto["Pendientes"];
