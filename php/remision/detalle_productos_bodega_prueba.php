<?php
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Headers: Origin, Content-Type, X-Auth-Token');
header('Content-Type: application/json');

require_once('../../config/start.inc.php');
include_once('../../class/class.lista.php');
include_once('../../class/class.complex.php');
include_once('../../class/class.consulta.php');

$id = ( isset( $_REQUEST['id'] ) ? $_REQUEST['id'] : '' );
$tipo = ( isset( $_REQUEST['tipo'] ) ? $_REQUEST['tipo'] : '' );

switch($tipo){
 case "Bodega":{
	$query = 'SELECT  PRD.Id_Producto, CONCAT(PRD.Principio_Activo," ",PRD.Presentacion," ",PRD.Concentracion," EMB-",PRD.Embalaje," (",PRD.Nombre_Comercial, ") ", PRD.Cantidad," ", 			 
	PRD.Unidad_Medida, ". ","LAB - ",PRD.Laboratorio_Generico 
	) as Nombre,  PRD.Nombre_Comercial, PRD.Laboratorio_Comercial, I.Id_Inventario, I.Costo as precio,PRD.Cantidad_Presentacion, CONCAT("{\'label\':", CONCAT("\'Lote: ", I.Lote, " - Vencimiento: ",  I.Fecha_Vencimiento," - Cantidad: ",(I.Cantidad-I.Cantidad_Apartada),"\'"),",\'value\':",I.Id_Inventario,",\'Codigo_Cum\':\'",I.Codigo_Cum,"\',\'Fecha_Vencimiento\':\'",I.Fecha_Vencimiento,"\',\'Lote\':\'",I.Lote,"\',\'Cantidad\':\'",I.Cantidad,"\',\'Costo\':\'",I.Costo,"\',\'Id_Inventario\':\'",I.Id_Inventario,"\',\'Cantidad_Apartada\':\'",I.Cantidad_Apartada,"\'}") as Lote
FROM Inventario I
   INNER JOIN Producto PRD
   On I.Id_Producto=PRD.Id_Producto   
   WHERE I.Id_Bodega='.$id.' 
   AND (I.Cantidad-I.Cantidad_Apartada)>0 AND I.Fecha_Vencimiento>"'.$nuevafecha.'"
   ORDER BY  PRD.Id_Producto, I.Fecha_Vencimiento ASC';
   break;
    }
    case "Punto":{
    $query = 'SELECT  PRD.Id_Producto, CONCAT(PRD.Principio_Activo," ",PRD.Presentacion," ",PRD.Concentracion," (",PRD.Nombre_Comercial, ") ", PRD.Cantidad," ", 			 
         PRD.Unidad_Medida, ". ","LAB - ",PRD.Laboratorio_Generico, " ") as Nombre,  PRD.Nombre_Comercial, PRD.Laboratorio_Comercial,I.Id_Inventario, I.Costo as precio,PRD.Cantidad_Presentacion, CONCAT("{\'label\':", CONCAT("\'Lote: ", I.Lote, " - Vencimiento: ",  I.Fecha_Vencimiento," - Cantidad: ",(I.Cantidad-I.Cantidad_Apartada),"\'"),",\'value\':",I.Id_Inventario,",\'Codigo_Cum\':\'",I.Codigo_Cum,"\',\'Fecha_Vencimiento\':\'",I.Fecha_Vencimiento,"\',\'Lote\':\'",I.Lote,"\',\'Cantidad\':\'",I.Cantidad,"\',\'Costo\':\'",I.Costo,"\',\'Id_Inventario\':\'",I.Id_Inventario,"\',\'Cantidad_Apartada\':\'",I.Cantidad_Apartada,"\'}") as Lote
	FROM Inventario I
    	INNER JOIN Producto PRD
    	On I.Id_Producto=PRD.Id_Producto   
    	WHERE I.Id_Punto_Dispensacion='.$id.' 
    	AND (I.Cantidad-I.Cantidad_Apartada)>0
   	 ORDER BY PRD.Id_Producto';
   	 break;
    } 
    
  } 
  
  //echo $query;     
$oCon= new consulta();
$oCon->setTipo('Multiple');
$oCon->setQuery($query);
$productos= $oCon->getData();
unset($oCon);


$i=-1;
$idproducto='';
$resultado=[];
$pos=-1;
$poslotes=0;
$lotes=[];
foreach($productos as $producto){ $i++;
	if ($producto['Id_Producto']!=$idproducto){
		if($pos>=0){
		   $resultado[$pos]["Lotes"]=$lotes;
		   $poslotes=0;
		}
		$pos++;
		$resultado[$pos]["Id_Producto"]=$producto["Id_Producto"];
		if($producto["Nombre"]==''){
			//var_dump($producto["Nombre_Comercial"]);
			//var_dump ($producto["Id_Producto"]);
			$resultado[$pos]["Nombre"]=$producto["Nombre_Comercial"]." LAB- ".$producto["Laboratorio_Comercial"];
		}else{
			$resultado[$pos]["Nombre"]=$producto["Nombre"];
		}
	
		
		$resultado[$pos]["precio"]=$producto["precio"];
		$resultado[$pos]["Id_Inventario"]=$producto["Id_Inventario"];
		$resultado[$pos]["Cantidad_Presentacion"]=$producto["Cantidad_Presentacion"];
		$idproducto=$producto['Id_Producto'];
		$lotes=[];
		$lotes[$poslotes]=(array) json_decode(str_replace("'",'"',$producto["Lote"]) , true);
	
	}else{
		$poslotes++;
		$lotes[$poslotes]=(array) json_decode(str_replace("'",'"',$producto["Lote"]) , true);
	 
	}
	
	

 
}

$resultado[$pos]["Lotes"]=$lotes;
	//var_dump($resultado);
echo json_encode($resultado);


?>