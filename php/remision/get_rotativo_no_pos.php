...
function CrearQuery(){
    ...
    $query='SELECT
    r.*, IFNULL(I.Cantidad_Disponible,0) AS Cantidad_Disponible, IFNULL(I.Precio,0) AS Precio
    FROM
    (
        SELECT
    ...
    SELECT
        Id_Producto,
        IFNULL(ROUND(AVG(Costo)),0) AS Precio,
        IFNULL(SUM(Cantidad-(Cantidad_Apartada+Cantidad_Seleccionada)),0) AS Cantidad_Disponible
        FROM Inventario
        WHERE Id_Bodega = '.$bodega.' 
        GROUP BY Id_Producto
    ) I ON r.Id_Producto = I.Id_Producto #HAVING Precio<'.$max_costo;
...
function SetCondiciones(){
    global $bodega,$eps,$fecha_inicio,$fecha_fin;
    ...
    $condicion .= " AND PRD.Id_Categoria IN (SELECT Id_Categoria FROM Bodega_Categoria WHERE Id_Bodega = $bodega)";
