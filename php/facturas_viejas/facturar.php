...
$query = 'SELECT 
            ...
            "" as observacion, 
            FV.Codigo as Codigo,
            "60" as Condicion_Pago , 
            DATE_ADD(FV.Fecha, INTERVAL 60 DAY) as Fecha_Pago, 
            "Factura" as tipo,
            C.Id_Cliente as IdCliente,
            C.Nombre as NombreCliente, 
            C.Direccion as DireccionCliente, 
            (SELECT Nombre FROM Municipio WHERE Codigo = CONCAT(FV.Departamento,FV.Ciudad)) as CiudadCliente, 
            C.Credito as CreditoCliente, 
            C.Cupo as CupoCliente,
            CONCAT_WS(" ",FV.Nombre_Paciente, CONCAT("- ", UPPER(IF(FV.Regimen="C","Contributivo","Subsidiado")))) AS Nombre_Paciente, 
            FV.Id_Paciente as Numero_Documento, 
            FV.Cuota_Recuperacion as Cuota,  
            FV.Codigo_Dis AS Cod_Dis, 
            FV.Tipo_Servicio AS Tipo_Servicio,
            "Cerrada" as Tipo_Valor
          FROM Z_Factura_Vieja FV
          INNER JOIN Cliente C ON FV.Id_Cliente = C.Id_Cliente
          WHERE FV.Codigo NOT IN('.$file.')
          GROUP BY FV.Codigo
...
try{
   ...
   $direc = $_SERVER["DOCUMENT_ROOT"]."php/facturas_viejas/FACTURAS/".$cliente["Codigo"].'.pdf'; // NOMBRE DEL ARCHIVO ES EL CODIGO DEL DOCUMENTO
