...
$oLista->SetRestrict("Id_Punto_Dispensacion","=",64);
$oLista->SetRestrict("Id_Producto","=",53865);
...
foreach($productos as $prod){ $k++;
    ...
    $tipo = 'Punto Dispensacion';
    $idTipo = $prod["Id_Punto_Dispensacion"];
    ...
    if ($tipo == 'Bodega') {
        ...
        WHERE INF.Estado="Terminado" AND PIF.Id_Producto = ' . $producto." AND PIF.Lote='".$prod["Lote"]."'" . ' AND INF.Id_Punto_Dispensacion = ' . $idTipo . ' GROUP BY PIF.Id_Producto) 
        ...
        WHERE D.Estado_Dispensacion != "Anulada" AND PD.Id_Producto = ' . $producto." AND PD.Lote='".$prod["Lote"]."'" . ' AND D.Id_Punto_Dispensacion = ' . $idTipo . ')';
    ...
    WHERE PAI.Id_Producto = '.$producto." AND PAI.Lote='".$prod["Lote"]."'".$condicion3.') 
    ...
    WHERE PAR.Id_Producto = '.$producto." AND PAR.Lote='".$prod["Lote"]."'".$condicion2. ') 
    
    UNION (SELECT INF.Id_Inventario_Fisico AS ID, "" AS Nombre_Origen, 
    (SELECT Nombre FROM Bodega WHERE Id_Bodega=INF.Bodega) AS Destino, 
    "inventariofisico/inventario_final_pdf.php" AS Ruta, 
    "Inventario" AS Tipo,
    CONCAT("INVF",INF.Id_Inventario_Fisico) AS Codigo, 
    INF.Fecha_Fin AS Fecha, SUM(PIF.Segundo_Conteo) AS Cantidad, 
    GROUP_CONCAT(PIF.Lote SEPARATOR " | ") AS Lote, 
    GROUP_CONCAT(PIF.Fecha_Vencimiento SEPARATOR " | ") AS Fecha_Vencimiento, "" AS Id_Factura, "" AS Codigo_Fact 
    FROM Producto_Inventario_Fisico PIF 
    INNER JOIN Inventario_Fisico INF 
    ON PIF.Id_Inventario_Fisico=INF.Id_Inventario_Fisico
    WHERE INF.Estado="Terminado" AND PIF.Id_Producto = '.$producto." AND PIF.Lote='".$prod["Lote"]."'".$condicion4. ' GROUP BY PIF.Id_Producto) '.$query_dispensaciones.' ORDER BY Fecha ASC';
   ...
   // echo $query;
    ...
    // if($prod["Cantidad"]!=(INT)$resultados[$i]["Saldo"]){ 
    //if(($prod["Cantidad"])!=(INT)$resultados[$i]["Saldo"]){ 
    $j++;
        /*if((INT)$resultados[$i]["Saldo"]<0){
            $resultados[$i]["Saldo"] = 0;
        }*/
        if($resultados[$i]["Saldo"]!=0 || $prod["Cantidad"]!=0){
        echo $k." - ". $j.") - ".$prod["Lote"]." - ".$prod["Id_Punto_Dispensacion"]." - ".$prod["Id_Producto"]." - ".$prod["Cantidad"]." - ".(INT)$resultados[$i]["Saldo"]." ".$prod["Id_Inventario"]."<br>";
