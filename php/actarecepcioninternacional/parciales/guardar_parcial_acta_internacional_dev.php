<?php
	...
	include_once('../../../class/class.contabilizar_dev.php');
	...
	$cod = $configuracion->Consecutivo('Parcial_Acta_Internacional');
    $modelo['Codigo']= $cod;
	...
	GuardarProductosParcial($productos, $id_nacionalizacion, $porcentaje_flete, $porcentaje_seguro, $porcentaje_flete_nac, $porcentaje_licencia);
	...
	function GuardarProductosParcial($productos, $id_parcial, $p_flete, $p_seguro, $p_flete_nac, $p_lic){
		...
		foreach ($productos as $p) {
	    	...
	    	$oItem->Adicional_Licencia_Importacion = number_format($p_lic, 4, ".", "");
	...
	function GuardarMovimientosContables($datos){
		...
		$contabilizar->CrearMovimientoContable('Parcial Acta Internacional', $datos);
	}
	...
	function GenerarNoConforme($producto, $id_acta){
		...
		$cod = $configuracion->Consecutivo('No_Conforme_Internacional');
