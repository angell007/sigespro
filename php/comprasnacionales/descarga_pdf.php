...
switch($tipo){
    case 'Orden_Compra_Nacional':{
        $query = 'SELECT 
        IFNULL(CONCAT(PRD.Principio_Activo," ",PRD.Presentacion," ",PRD.Concentracion," ", PRD.Cantidad," ", PRD.Unidad_Medida),CONCAT(PRD.Nombre_Comercial," ",PRD.Laboratorio_Comercial)) as Nombre_Producto, 
    ...
    WHERE POCN.Id_Orden_Compra_Nacional ='.$id ;
        ...
        <table style="font-size:10px;margin-top:10px;" cellpadding="0" cellspacing="0">
            <thead>
            <tr>
                <th style="width:280px;max-width:270px;font-weight:bold;background:#cecece;;border:1px solid #cccccc;">
                    Producto
                </th>
                <th style="width:130px;font-weight:bold;background:#cecece;text-align:center;border:1px solid #cccccc;">
                   Embalaje
                </th>
                <th style="width:80px;font-weight:bold;background:#cecece;text-align:center;border:1px solid #cccccc;">
                   Cantidad
                </th>
                <th style="width:80px;font-weight:bold;background:#cecece;text-align:center;border:1px solid #cccccc;">
                   Costo
                ...
                <th style="width:60px;font-weight:bold;background:#cecece;text-align:center;border:1px solid #cccccc;">
                    SubTotal
                </th>
            </tr>
            </thead>
            ...
            foreach ($productos as $prod) {
                ...
                $color=$prod["Costo_Promedio"]>$prod["Costo"]?'#a7a7a7':'#f3f3f3'; 
                $contenido .= '
                    <tr>
                    <td style="width:280px; max-width:280px;background:#f3f3f3; padding:3px 2px;font-size:9px;text-align:left;vertical-align:middle;border:1px solid #cccccc;word-break: break-all !important; word-wrap: break-word!important;"><b>'.$prod["Nombre_Comercial"] .'</b>
                    <br><span style="color:gray">'.$prod["Nombre_Producto"].'</span>
                    <br><span style="color:gray">CUM:'.$prod["Codigo_Cum"].'</span>
                    </td>
                    <td style="width:130px; max-width:130px;font-size:9px;vertical-align:center;word-wrap: break-word;text-align:left;background:#f3f3f3;border:1px solid #cccccc;"><p style="font-size:7px;color:gray">'.$prod["Embalaje"].'</p></td>
                    <td style="max-width:80px;font-size:9px;vertical-align:middle;word-wrap: break-word;text-align:center;background:#f3f3f3;border:1px solid #cccccc;">'.number_format($prod["Cantidad"],0,",",".").'</td>
                    <td style="max-width:80px;font-size:9px;vertical-align:middle;word-wrap: break-word;text-align:center;background:'.$color.';border:1px solid #cccccc;">$'.number_format($prod["Costo"],2,",",".").'</td>
                    <td style="max-width:80px;font-size:9px;vertical-align:middle;word-wrap: break-word;text-align:center;background:#f3f3f3;border:1px solid #cccccc;">'.$prod["Iva"].'%</td>
                    <td style="max-width:60px;font-size:9px;vertical-align:middle;word-wrap: break-word;text-align:center;background:#f3f3f3;border:1px solid #cccccc;">$'.number_format($prod["Total"],2,",",".").'</td>';
                    $contenido .= '</tr>';
	...
	<td style="width:740px;border:1px solid #cccccc;">
		<strong>Persona Elabora</strong><br><br><br><br><br><br><br>
...
try{
    ...
    $html2pdf->Output($direc,'L'); // LA D SIGNIFICA DESCARGAR, 'F' SE PODRIA HACER PARA DEJAR EL ARCHIVO EN UNA CARPETA ESPECIFICA
