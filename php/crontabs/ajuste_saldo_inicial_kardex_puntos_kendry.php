...
FROM Inventario WHERE  Id_Punto_Dispensacion =$punto_select  
/* AND Id_Producto=22000*/
ORDER BY Id_Punto_Dispensacion, Id_Producto ";
...
foreach($productos as $prod){ $k++;
 ...
 //echo $sql_acta_recepcion_bodegas; exit;
        ...
        WHERE INF.Estado="Terminado" AND PIF.Id_Producto = ' . $producto." AND ( PIF.Lote='".$prod["Lote"]."'" . ') AND INF.Id_Punto_Dispensacion = ' . $idTipo . ' AND INF.Fecha_Fin BETWEEN "'.$fecha1.' 00:00:00" AND "'.$fecha2.' 23:59:59" GROUP BY PIF.Id_Producto, INF.Fecha_Fin) 
        ...
        D.Codigo, IFNULL((SELECT PDP.Timestamp FROM Producto_Dispensacion_Pendiente PDP WHERE PDP.Id_Producto_Dispensacion=PD.Id_Producto_Dispensacion
        ...
        LIMIT 1),D.Fecha_Actual)) AS Fecha, PD.Cantidad_Entregada AS Cantidad, PD.Lote, "" AS Fecha_Vencimiento, "" AS Id_Factura,
        ...
        WHERE D.Estado_Dispensacion != "Anulada" AND  PD.Cantidad_Entregada!=0 AND PD.Id_Producto = ' . $producto." AND PD.Id_Inventario=".$prod["Id_Inventario"]."" . ' AND I.Id_Punto_Dispensacion = ' . $idTipo . ' 
    ...
    foreach($resultados as $res){ $i++;
     ...
     echo $i." )". $res['Codigo']." (".$res["ID"].") &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp ".$res['Tipo']." : ".$res['Cantidad']
     ." &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Fecha: --- ".$res['Fecha']
     ." &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Saldo: ".$acum
     ." &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp ID_Inventario :".$prod['Id_Inventario']
     ." &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Id_Punto: ".$prod['Id_Punto_Dispensacion']."<br>";
        $resultados[$i]["Saldo"]=$acum;
    }
    
    
     
        echo "<strong>".$k." - ". $j.") - ".$prod["Lote"]." - ".$idTipo." - ".$prod["Id_Producto"]." - ".$prod["Cantidad"]." : (".(INT)$resultados[$i]["Saldo"]." Entradas= ".$entrada." Salidas: --(".$salida."</strong>\n <br> <br>";
