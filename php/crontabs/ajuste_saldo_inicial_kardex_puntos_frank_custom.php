...
ORDER BY Id_Punto_Dispensacion, Id_Producto, Id_Inventario /*LIMIT 110,25*/";
...
foreach($productos as $prod){ $k++;
 ...
 //echo $sql_acta_recepcion_bodegas; exit;
    ...
    $texto2 = "<strong>".$k." - ". $j.") - ".$prod["Lote"]." - ".$idTipo." - ".$prod["Id_Producto"]." - ".$prod["Cantidad"]." : (".(INT)$resultados[$i]["Saldo"]." Entradas= ".$entrada." Salidas: --(".$salida." - Costo Inv: ".$acum_inv."</strong>";
        $resultados[$i]['Negativo']='No';

    if($resultados[$i]["Saldo"]!=$prod["Cantidad"]){
        $texto2 .= "  <-------- <strong>( DIFERENTE )</strong> <br><br>";
