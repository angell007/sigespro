...
$queryinventario='UPDATE Inventario_Nuevo SET Cantidad = 0, Cantidad_Apartada = 0, Cantidad_Seleccionada = 0
...
foreach ($inventarios as $item) {
    ...
    foreach($resultado as $res){ $i++;
        $query = 'SELECT Id_Inventario_Nuevo,Cantidad FROM Inventario_Nuevo WHERE Id_Producto='.$res["Id_Producto"].' AND Lote="'.$res["Lote"].'" AND Fecha_Vencimiento="'.$res['Fecha_Vencimiento'].'" AND Id_Punto_Dispensacion='.$item['Id_Punto_Dispensacion'];
...
if($id_inventario){
    $datos_movimiento_contable['Id_Registro'] = $datos['Id_Inventario_Fisico'];
...
function getCosto($id_producto){
    $query="SELECT Costo_Promedio FROM Costo_Promedio WHERE Id_Producto=".$id_producto.'  ';
    $oCon= new consulta();
    $oCon->setQuery($query);
    $costo = $oCon->getData();
    unset($oCon);
