...
function getCampo() {
    ...
    $campo['codigo'] = 'Codigo';
    $campo['nombre'] = 'Nombre_PCGA';
    $campo['debe'] = 'Debe_PCGA';
    $campo['haber'] = 'Haber_PCGA';
  ...
  function obtenerSaldoAnterior($naturaleza, $array, $index, $tipo_nit = false)
{
    ...
    $fecha_ini = isset($fechas[0])?$fechas[0]: $_REQUEST['Fecha_Inicial'];
...
function getMovimientosCuenta($fecha1, $fecha2, $nit = null, $plan = null)
{
    ...
    if ($nit === null) {
        ...
        $query = "SELECT MC.Id_Plan_Cuenta, SUM(Debe) AS Debito_PCGA, SUM(Haber) AS Credito_PCGA, SUM(Debe_Niif) AS Debito_NIIF, SUM(Haber_Niif) AS Credito_NIIF FROM Movimiento_Contable MC WHERE DATE(Fecha_Movimiento) BETWEEN '$fecha1' AND '$fecha2' AND MC.Nit = $nit AND MC.Id_Plan_Cuenta = $plan AND MC.Estado != 'Anulado'";
