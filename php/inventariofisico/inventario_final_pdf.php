...
/* FIN HOJA DE ESTILO PARA PDF*/
        ...
        $query = 'SELECT PIF.Id_Producto_Inventario_Fisico, PIF.Id_Inventario_Fisico, PIF.Id_Inventario, P.Nombre_Comercial, CONCAT(P.Principio_Activo," ",P.Presentacion," ",P.Concentracion, P.Cantidad," ", P.Unidad_Medida, " ") AS Nombre_Producto, PIF.Lote, PIF.Fecha_Vencimiento, PIF.Primer_Conteo AS Cantidad_Encontrada, IFNULL(PIF.Segundo_Conteo,PIF.Primer_Conteo) AS Segundo_Conteo, PIF.Cantidad_Inventario,
        (PIF.Cantidad_Inventario-PIF.Segundo_Conteo) AS Cantidad_Diferencial FROM Producto_Inventario_Fisico PIF INNER JOIN Producto P ON PIF.Id_Producto=P.Id_Producto WHERE PIF.Id_Inventario_Fisico IN ('.$id_inventario_fisico . ') ORDER BY P.Nombre_Comercial';
        ...
        $codigos ='
            ...
            <h4 style="margin:5px 0 0 0;font-size:22px;line-height:22px;">INVF'.$datos['Id_Inventario_Fisico'].'</h4>
            <h6 style="margin:5px 0 0 0;font-weight: normal;font-size:16px;line-height:16px;">Inicio: '.$datos['f_inicio'].'</h6>
            <h6 style="margin:5px 0 0 0;font-weight: normal;font-size:16px;line-height:16px;">Fin: '.$datos['f_fin'].'</h6>
            <h6 style="margin:5px 0 0 0;font-weight: normal;font-size:16px;line-height:16px;">Bod.: '.$datos['Nom_Bodega'].'</h6>
            <h6 style="margin:5px 0 0 0;font-weight: normal;font-size:16px;line-height:16px;">Cat.: '.$datos['Nom_Categoria'].'</h6>
            <h6 style="margin:5px 0 0 0;font-weight: normal;font-size:16px;line-height:16px;">Letras: '.$datos['Letras'].'</h6>
        ';

        $contenido = '
        <table style="font-size:10px;margin-top:10px" cellpadding="0" cellspacing="0">
            <tr>
                <td style="width:236px;max-width:236px;font-weight:bold;background:#cecece;;border:1px solid #cccccc;">
                 Funcionario Digitador
                </td>
                <td style="width:236px;max-width:236px;font-weight:bold;background:#cecece;;border:1px solid #cccccc;">
                    Funcionario Contador
                </td>
                <td style="width:236px;font-weight:bold;background:#cecece;text-align:center;border:1px solid #cccccc;">
                    Funcionario Autorizó
                </td>
            </tr>
            <tr>
                ...
                <td style="width:236px;font-size:9px;vertical-align:middle;background:#f2f2f2;word-wrap: break-word;text-align:center;border:1px solid #cccccc;">'.$datos["Funcionario_Autorizo"].'</td>
            </tr>
        </table>
        ';
        ...
        <table style="font-size:10px;margin-top:20px;" cellpadding="0" cellspacing="0">
            ...
            foreach ($productos as $i => $prod) {
                ...
                <td style="width:60px;font-size:9px;vertical-align:middle;word-wrap: break-word;text-align:center;border:1px solid #cccccc;">'.$prod["Cantidad_Inventario"].'</td>
