...
$query = 'SELECT * FROM Doc_Inventario_Fisico I WHERE I.Id_Doc_Inventario_Fisico='.$id_doc_inventario_fisico ;
...
if($Inv['Estado']=='Primer Conteo'){
    ...
    $producto_inventario_fisico=GetProductos($Inv['Id_Doc_Inventario_Fisico']);
    $producto_no_contados=GetProductosNoContados($Inv['Id_Doc_Inventario_Fisico']);
    
    $ids_prod_inv_fisico;
    foreach ($producto_inventario_fisico as $key => $value) {
        $ids_prod_inv_fisico.=$value['Id_Producto_Doc_Inventario_Fisico'] . ' ,';
    }
    ...
    $prodcuto_no_inventario=GetProductosNoInventario($Inv['Id_Doc_Inventario_Fisico'],$ids_prod_inv_fisico);
     ...
     $resultado['Inventarios']=$Inv['Id_Doc_Inventario_Fisico'];
...
function GetProductos($inventarios){
    ...
    $query = 'SELECT
    GROUP_CONCAT(
        PIF.Id_Producto_Doc_Inventario_Fisico
    ) AS Id_Producto_Doc_Inventario_Fisico,
    PIF.Id_Doc_Inventario_Fisico, PIF.Id_Inventario_Nuevo, P.Nombre_Comercial,
    CONCAT(
        P.Principio_Activo,
        " ",
        P.Presentacion,
...
WHERE PIF.Id_Doc_Inventario_Fisico IN('.$inventarios.')
...
function GetProductosNoContados ($inventarios){
    global $Inv;
    $query_inventario='SELECT PIF.Id_Inventario_Nuevo FROM Producto_Doc_Inventario_Fisico PIF
    WHERE PIF.Id_Doc_Inventario_Fisico IN ('.$inventarios.') ';
 
   
    $query='SELECT CONCAT("-",I.Cantidad) as Cantidad_Diferencial, 0 as Cantidad_Encontrada, I.Fecha_Vencimiento, I.Id_Producto, 0 as Id_Producto_Doc_Inventario_Fisico, I.Lote, P.Nombre_Comercial, CONCAT_WS(" ", P.Principio_Activo,P.Presentacion,P.Concentracion, P.Cantidad, P.Unidad_Medida, " LAB1: ", P.Laboratorio_Comercial, " LAB2: ",P.Laboratorio_Generico) AS Nombre_Producto, I.Cantidad as Segundo_Conteo, I.Cantidad as Cantidad_Inventario, I.Id_Inventario_Nuevo
...
function GetProductosNoInventario($id_inv_fisico,$id_productos){
    ...
    if ($id_productos) {
        $cond='WHERE PIF.Id_Producto_Doc_Inventario_Fisico NOT IN ('.$id_productos.') 
                 AND PIF.Id_Doc_Inventario_Fisico IN ('.$id_inv_fisico.')
                 ORDER BY P.Nombre_Comercial';
     }else{
         $cond='WHERE PIF.Id_Doc_Inventario_Fisico IN ('.$id_inv_fisico.')
         ORDER BY P.Nombre_Comercial';
     }
         ...
         PIF.Id_Producto, PIF.Id_Producto_Doc_Inventario_Fisico, PIF.Lote, P.Nombre_Comercial, CONCAT_WS("", P.Principio_Activo,P.Presentacion,
