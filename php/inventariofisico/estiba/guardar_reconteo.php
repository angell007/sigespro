...
foreach ($listado_inventario as $value) {
    ...
    if($value['Id_Producto_Doc_Inventario_Fisico']!=0){
        ...
        for ($i=0; $i < count( $id_inventario) ; $i++) { 
            if($i!=0){
                $oItem = new complex('Producto_Doc_Inventario_Fisico', 'Id_Producto_Doc_Inventario_Fisico', $id_inventario[$i]);
                $oItem->delete();
                unset($oItem);
             }else{
                $oItem = new complex('Producto_Doc_Inventario_Fisico', 'Id_Producto_Doc_Inventario_Fisico', $id_inventario[$i]);
...
WHERE  Id_Doc_Inventario_Fisico IN ('.$inventarios.')';
$oCon= new consulta();
...
$query2='UPDATE Producto_Doc_Inventario_Fisico
SET Segundo_Conteo = Primer_Conteo
WHERE Segundo_Conteo IS NULL AND Id_Doc_Inventario_Fisico IN ('.$inventarios.')';
$oCon= new consulta();
$oCon->setQuery($query2);     
$oCon->createData();     
unset($oCon);
