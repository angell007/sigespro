...
foreach($dispensaciones as $dis){
    $cabecera = $contenido1 = $contenido = '';
    $id = $dis["Id_Dispensacion"];
    $ruta = '';
...
if ($_REQUEST["productos"]) {
    ...
    // (SELECT A.Producto FROM A_Entrega_Nutriciones_2025 A WHERE A.id_producto = PD.Id_Producto LIMIT 1)
    
    $query = "SELECT PD.*, Producto.Nombre_Comercial as Nombre_Producto, Producto.Codigo_Cum as Cum,
    Producto.Invima
...
if (strpos($encabezado["Tipo"], 'MIPRES') !== false) {
    ...
    <tr>
        ...
        <td style="font-weight:bold;font-size:10px;width:150px;background:#f3f3f3;vertical-align:middle;padding:6px;" colspan="4">
        Observaciones
        </td>
    </tr>
    <tr>
    <td style="font-size:7px;width:176px;background:#f3f3f3;vertical-align:middle;padding:6px;">
    ' . $encabezado['Numero_Prescripcion'] . '
    </td>
    <td style="font-size:7px;width:150px;background:#f3f3f3;vertical-align:middle;padding:6px;" colspan="4">
...
if ($encabezado['Firma'] != '') {
    $firma = '<img src="' . $MY_FILE . "DOCUMENTOS/" . $encabezado["Funcionario1"] . "/" . $encabezado['Firma'] . '"  style="max-height=90px; max-width= 200px;"> <br>';
} else {
...
$content.= '<page backtop="150px" backbottom="5px" style="width:215.9mm;height:170mm;"' . $marca_agua . '>
            <page_header>' .
    $cabecera . $contenido1 .
    '</page_header>
...
function getDispensaciones()
{
	...
	$query_productos =
		'SELECT *
		    FROM Dispensacion D
		    WHERE D.Fecha_Actual LIKE "%2025-03-19%"
		';
