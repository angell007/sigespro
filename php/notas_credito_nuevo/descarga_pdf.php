...
switch($tipo){
    case 'Nota_Credito':{
        ...
        $query= 'SELECT * FROM Nota_Credito_Global 
            WHERE Id_Nota_Credito_Global = ' .$id;
    $oCon = new consulta();
    $oCon->setQuery($query);
    ...
    /*     foreach ($descripciones_nota as $key => $descripcion) {
        ...
        $codigos ='
            ...
            <h5 style="margin:5px 0 0 0;font-size:16px;line-height:16px;">'.fecha($Nota_Credito["Fecha"]).'</h5>
                ...
                <td style="width:720px; padding-right:0px;">
                    <table cellspacing="0" cellpadding="0" style="text-transform:uppercase;">
                        ...
                        <tr>
                            ...
                            <td style="font-size:10px;width:175px;background:#f3f3f3;border:1px solid #cccccc;text-align:center;">
                            '.fecha($Nota_Credito['Factura']['Fecha_Documento']).'
                            </td>
                        </tr>
                    </table>
                </td>
		...
		<td style="width:10px;background:#cecece;;border:1px solid #cccccc;"></td>
            ...
            $total = 0;
            $total_iva = 0;
            foreach($Productos_Nota as $prod){  $max++;
                $contenido .='<tr>
...
try{
    ...
    $direc = $Nota_Credito["Codigo"].'.pdf'; // NOMBRE DEL ARCHIVO ES EL CODIGO DEL DOCUMENTO
