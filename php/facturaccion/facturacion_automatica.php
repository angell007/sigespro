<?php
  ...
  require_once('../../config/start.inc.php');
   ...
   //$contabilizar = new Contabilizar();
    ...
    $response = array();
     $query='SELECT D.Id_Dispensacion 
      FROM Dispensacion D
    WHERE
    D.Codigo IN () AND D.Estado_Facturacion!="Facturada" AND D.Estado_Dispensacion!="Anulada" ';                         
   ...
   function GetDispensacion($id){
       global $queryObj;
        $query="SELECT 
        D.Codigo as Codigo, Dep.Nombre, P.EPS, P.Nit, DC.Id_Cliente, Dep.Id_Departamento , CONCAT(P.Id_Paciente , ' - ', P.Primer_Nombre, ' ', P.Primer_Apellido,  ' - Regimen ' , R.Nombre ) as Paciente, 'Cohortes' AS Tipo_Dispensacion, P.Id_Regimen, D.Cuota
        FROM `Dispensacion` D       
     ...
     global $contabilizar;
      ...
      switch($tipo){
          case "Factura":{
              ...
              $oItem = new complex('Resolucion','Id_Resolucion',15); // Resolucion 3 para Facturas Ventas NoPos
              $nc = $oItem->getData();
