<?php
	...
	/* $http_response->SetRespuesta(2, 'Alerta', 'En estos momentos se está realizando mantenimiento a la plataforma de radicados, qué pena las molestias causadas.');
	...
	if ($modelo['Id_Radicado'] == '') {
		...
		// $tipo_servicio = $modelo['Tipo_Servicio'] == '' ? "Todos" : $modelo['Tipo_Servicio'];
		...
		GuardarActividadFactura($id_radicado, $dataRadicado['Id_Funcionario'], $dataRadicado['Codigo']);
	...
	function GuardarActividadFactura($idRadicado,$idFuncionario,$codigo){
		...
		foreach ($facturas as $i){
			...
			$oItem->Detalle        = "Se radica factura ". $i['Codigo_Factura'] ." al radicado ".$codigo;
