...
$oItem = new complex($tipo,"Id_Remision",$id);
$data = $oItem->getData();
unset($oItem);
...
switch($tipo){
    case 'Remision_Antigua':{
        $query = 'SELECT PR.Lote, I.Fecha_Vencimiento, IFNULL(CONCAT( P.Principio_Activo, " ",
P.Presentacion, " ",
P.Concentracion, " (", P.Nombre_Comercial,") ",
P.Cantidad," ",
P.Unidad_Medida, " LAB-", P.Laboratorio_Comercial ), CONCAT(P.Nombre_Comercial," LAB-", P.Laboratorio_Comercial)) AS Nombre_Producto, PR.Cantidad, PR.Precio, PR.Descuento, PR.Impuesto, PR.Subtotal
...
WHERE PR.Id_Remision='.$id.' AND PR.Lote=I.Lote
...
ORDER BY Nombre_Producto';
		...
		<td style="width:10px;background:#cecece;;border:1px solid #cccccc;"></td>
            ...
            foreach($productos as $prod){  $max++;
                $contenido .='<tr>
                    ...
                    <td style="padding:3px 2px;width:500px;max-width:500px;font-size:9px;text-align:left;background:#f3f3f3;border:1px solid #cccccc;word-break: break-all !important;">'.$prod["Nombre_Producto"].'</td>
	...
	<td style="width:240px;border:1px solid #cccccc;">
		<strong>Persona que Elabora</strong><br><br><br><br><br><br><br>
