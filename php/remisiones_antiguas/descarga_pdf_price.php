...
switch($tipo){
    case 'Remision':{
        $query = 'SELECT PR.Lote, PR.Fecha_Vencimiento, PR.Nombre_Producto, PR.Cantidad, PR.Precio, PR.Descuento, PR.Impuesto, PR.Subtotal
		...
		<td style="width:10px;background:#cecece;;border:1px solid #cccccc;"></td>
            ...
            foreach($productos as $prod){  $max++;
                $contenido .='<tr>
                    ...
                    <td style="width:50px;font-size:9px;word-wrap: break-word;text-align:center;background:#f3f3f3;border:1px solid #cccccc;">'.$prod["Precio"].'</td>
