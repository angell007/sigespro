...
$query = "SELECT AI.*,  CONCAT(F.Nombres,' ', F.Apellidos) AS Funcionario, IF(AI.Origen_Destino = 'Bodega',(SELECT B.Nombre FROM Bodega B WHERE B.Id_Bodega=AI.Id_Origen_Destino),(SELECT B.Nombre FROM Punto_Dispensacion B WHERE B.Id_Punto_Dispensacion=AI.Id_Origen_Destino)) as Bodega FROM Ajuste_Individual AI
...
$query = "SELECT AI.*,  CONCAT(F.Nombres,' ', F.Apellidos) AS Funcionario, IF(AI.Origen_Destino = 'Bodega',(SELECT B.Nombre FROM Bodega B WHERE B.Id_Bodega=AI.Id_Origen_Destino),(SELECT B.Nombre FROM Punto_Dispensacion B WHERE B.Id_Punto_Dispensacion=AI.Id_Origen_Destino)) as Bodega,
...
INNER JOIN Funcionario F 
ON F.Identificacion_Funcionario=AI.Identificacion_Funcionario ".$condicion.' '.$condicion_bod.' ORDER BY AI.Fecha DESC LIMIT '.$limit.','.$tamPag;
