...
$condicion = '';
$condicion2 = '';
//validar si no hay campos de busqueda únicos de Acta Recepcion 
//el estado de la acta recepcion (aprobada-acomodada)
  $cond_estado = '';
/*if (isset($_REQUEST['estado']) && $_REQUEST['estado'] != "") {
    $estado =$_REQUEST['estado'];
    if($_REQUEST['estado'] == 'Acomodada'){
    $cond_estado = 'AND ( (ARC.Fecha_Creacion > "22-07-2020" AND ARC.Estado = "Acomodada") OR (ARC.Fecha_Creacion > "2020-07-22" AND ARC.Estado = "Aprobada") )';
    }else{
         $cond_estado=' AND ARC.Estado =  "'.$estado.'"';
    }
}*/
...
WHERE   ( (ARC.Fecha_Creacion > "2020-07-22" AND ARC.Estado = "Acomodada") OR (ARC.Fecha_Creacion > "2020-07-22" AND ARC.Estado = "Aprobada") )  AND ARC.Tipo_Acta = "Bodega" '.$condicion;
...
$query='SELECT COUNT(*) AS Total
        ...
        WHERE ARC.Tipo="Bodega"  AND  ARC.Estado = "'.$estado.'" '.$condicion2;
        //    WHERE ARC.Tipo="Bodega" AND ARC.Id_Bodega IN ('.$bodegas['Id_Bodega'].')'.$condicion; 
        $oCon= new consulta();
        $oCon->setQuery($query);
...
####### PAGINACIÓN ######## 
$tamPag = 10; 
$numReg = $total_acta_recepcion_remision["Total"]  + $total['Total']; 
...
$query = 'SELECT ARC.Id_Acta_Recepcion, ARC.Codigo, ARC.Fecha_Creacion, F.Imagen, IFNULL(B.Nombre,BV.Nombre) as Bodega, OCN.Codigo as Codigo_Compra_N, P.Nombre as Proveedor,
...
INNER JOIN Proveedor P
ON P.Id_Proveedor = ARC.Id_Proveedor
WHERE   ( (ARC.Fecha_Creacion > "2020-07-22" AND ARC.Estado = "Acomodada") OR (ARC.Fecha_Creacion < "2020-07-22" AND ARC.Estado = "Aprobada") ) AND    ARC.Tipo_Acta = "Bodega " 
...
if ($Actas_Recepcion_Remision) {
    # code...
    $actarecepcion['actarecepciones']=array_merge($actarecepcion['actarecepciones'],$Actas_Recepcion_Remision);
}
