<?php
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Headers: Origin, Content-Type, X-Auth-Token');
header('Content-Type: application/json');

require_once('../../config/start.inc.php');
include_once('../../class/class.lista.php');
include_once('../../class/class.complex.php');
include_once('../../class/class.consulta.php');

$tipoCompra = ( isset( $_REQUEST['compra'] ) ? $_REQUEST['compra'] : '' );
$tipo = ( isset( $_REQUEST['tipo'] ) ? $_REQUEST['compra'] : '' );

$condicion = '';

if (isset($_REQUEST['cod']) && $_REQUEST['cod'] != "") {
    $condicion .= " AND OC.Codigo LIKE '%$_REQUEST[cod]%'";
}
if (isset($_REQUEST['proveedor']) && $_REQUEST['proveedor'] != "") {
    $condicion .= " AND P.Nombre LIKE '%$_REQUEST[proveedor]%'";
}
if (isset($_REQUEST['punto']) && $_REQUEST['punto'] != "") {
    $condicion .= " AND OC.Id_Punto_Dispensacion='$_REQUEST[punto]'";
}else{
    $condicion .= " AND OC.Id_Punto_Dispensacion=0";
}

switch($tipoCompra){
    
    case "Nacional":{
        $query = '
            SELECT OC.Id_Orden_Compra_Nacional, OC.Fecha, F.Imagen, P.Nombre , OC.Codigo,
            (SELECT COUNT(*) FROM Producto_Orden_Compra_Nacional PR WHERE PR.Id_Orden_Compra_Nacional = OC.Id_Orden_Compra_Nacional) as Items,
            P.Id_Plan_Cuenta_Retefuente,
            P.Id_Plan_Cuenta_Reteiva,
            P.Id_Plan_Cuenta_Reteica,
            CONCAT_WS(" ", P.Primer_Nombre,P.Segundo_Nombre,P.Primer_Apellido,P.Segundo_Apellido) AS Proveedor,
            P.Razon_Social,
            P.Tipo_Retencion,
            P.Tipo_Reteica,
            OC.Estado
            FROM Orden_Compra_Nacional OC
            LEFT JOIN Funcionario F ON OC.Identificacion_Funcionario = F.Identificacion_Funcionario
            LEFT JOIN Proveedor P ON OC.Id_Proveedor = P.Id_Proveedor
            WHERE 
                OC.Estado <> "Recibida" 
                AND OC.Estado <> "Anulada" 
                AND OC.Aprobacion = "Aprobada" 
                AND OC.Id_Bodega != 0  
                AND P.Tipo_Retencion <> "Autorretenedor"
                AND P.Tipo_Reteica IS NOT NULL AND P.Tipo_Reteica <> "N/A"
                AND (P.Id_Plan_Cuenta_Retefuente IS NOT NULL || P.Id_Plan_Cuenta_Reteiva IS NOT NULL) '.$condicion.'
            ORDER BY OC.Fecha DESC, OC.Codigo DESC LIMIT 20' ;
        break;
 }
    case "Internacional":{
        $query = 'SELECT OC.Id_Orden_Compra_Internacional, OC.Fecha, F.Imagen, P.Nombre , OC.Codigo,
(SELECT COUNT(*) FROM Producto_Orden_Compra_Internacional PR WHERE PR.Id_Orden_Compra_Internacional = OC.Id_Orden_Compra_Internacional) as Items
FROM Orden_Compra_Internacional OC
LEFT JOIN Funcionario F
ON OC.Identificacion_Funcionario = F.Identificacion_Funcionario
LEFT JOIN Proveedor P
ON OC.Id_Proveedor = P.Id_Proveedor
WHERE OC.Estado <> "Recibida" AND OC.Estado <> "Anulada" 
ORDER BY OC.Fecha DESC, OC.Codigo DESC' ;
        break;
    }
}

$oCon= new consulta();
$oCon->setQuery($query);
$oCon->setTipo('Multiple');
$resultado = $oCon->getData();
unset($oCon);

echo json_encode($resultado);
          
?>