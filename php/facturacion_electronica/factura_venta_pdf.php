...
$contenido = '<table  cellspacing="0" cellpadding="0" >
			    ...
			    $total_iva = 0;
	        	    foreach($productos as $prod){ $total_iva += ($prod["Subtotal"]*(str_replace("%","",$prod["Impuesto"])/100));
	        	    	$contenido.='<tr>
	             	   ...
	             	   <td rowspan="3" style="padding:4px;font-size:9px;border:1px solid #c6c6c6;width:130px;">
	             	   	<table cellpadding="0" cellspacing="0">
	             	   	   <tr>
	             	   		...
	             	   		<td style="padding:4px;font-size:9px;width:60px;text-align:right;">$ '.number_format($totalFactura['TotalFac'],0,",",".").
'</td>
	             	   	   </tr>
	             	   	   <tr>
	             	   	   ...
	             	   	   <tr>
	             	   		...
	             	   		<td style="padding:4px;font-size:9px;width:60px;text-align:right;">$ ' . number_format($total_iva, 0, ",", ".") .
'</td>
	             	   	   </tr>
	             	   	   <tr>
	             	   	   ...
	             	   	   <tr>
	             	   		...
	             	   		<td style="padding:4px;font-size:9px;width:60px;text-align:right;"><strong>$ ' . number_format($totalFactura['TotalFac'] + $total_iva, 0, ",", ".") .
'</strong></td>
	             	   	   </tr>
	             	   	</table>
	             	   </td>
...
$content = '<page backtop="290px" backbottom="185px" '.$marca_agua.'>
...
try{
   ...
   $direc = $ruta; // NOMBRE DEL ARCHIVO ES EL CODIGO DEL DOCUMENTO
    ...
    $html2pdf->Output($_SERVER['DOCUMENT_ROOT'] . $ruta, "F"); // LA D SIGNIFICA DESCARGAR, 'F' SE PODRIA HACER PARA DEJAR EL ARCHIVO EN UNA CARPETA ESPECIFICA
} catch (\Throwable $e) {
    echo $e;
    exit;
}
