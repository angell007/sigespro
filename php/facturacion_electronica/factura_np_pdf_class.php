...
class FacturaVentaPdf
{
    ...
    public function generarPdf()
    {
        ...
        $oItem = new complex("Factura", "Id_Factura", $this->id);
        $data = $oItem->getData();
        unset($oItem);
        ...
        $query = 'SELECT FV.Cufe,
            D.Id_Servicio, FV.Id_Factura, FV.Id_Factura_Asociada, FV.Codigo_Qr, FV.Fecha_Documento as Fecha , FV.Observacion_Factura as observacion, FV.Codigo as Codigo,IF(FV.Condicion_Pago=1,"CONTADO",FV.Condicion_Pago) as Condicion_Pago , FV.Fecha_Pago as Fecha_Pago , FV.Tipo as tipo,
            C.Id_Cliente as IdCliente ,C.Nombre as NombreCliente, C.Direccion as DireccionCliente, (SELECT Nombre FROM Municipio WHERE Id_Municipio = C.Ciudad) as CiudadCliente, C.Credito as CreditoCliente, C.Cupo as CupoCliente,
            ...
            D.Numero_Documento, FV.Cuota, D.Codigo AS Cod_Dis, (SELECT Nombre FROM Tipo_Servicio WHERE Id_Tipo_Servicio = D.Id_Tipo_Servicio) AS Tipo_Servicio, C.Tipo_Valor,
            (SELECT  P.Eps FROM Dispensacion D INNER JOIN (SELECT EPS, Id_Paciente FROM Paciente ) P ON D.Numero_Documento=P.Id_Paciente   WHERE Id_Dispensacion=FV.Id_Dispensacion) as Eps
          FROM Factura FV
          INNER JOIN Dispensacion D ON FV.Id_Dispensacion = D.Id_Dispensacion 
          INNER JOIN Cliente C
           ON FV.Id_Cliente = C.Id_Cliente
          WHERE FV.Id_Factura = ' . $this->id;
       ...
       ON I.Id_Inventario = PD.Id_Inventario  WHERE PFV.Id_Factura =  ' . $this->id;
        $oCon = new consulta();
        $oCon->setQuery($query);
        ...
        $query5 = 'SELECT SUM(Subtotal) as TotalFac FROM Producto_Factura WHERE Id_Factura = ' . $this->id;
        ...
        $titulo = $cliente["tipo"] == 'Homologo' ? 'Factura Electrónica de Venta (HOMOLOGO)' : 'Factura Electrónica de Venta ' . ($cliente['tipo'] == 'Homologo' ? '(HOMOLOGO)' : '');
        ...
        $codigos = '
            ...
            <h5 style="margin:5px 0 0 0;font-size:8px;line-height:8px;">F. Expe.:' . $this->fecha($cliente["Fecha"]) . '</h5>
            <h5 style="margin:5px 0 0 0;font-size:8px;line-height:8px;">H. Expe.:&nbsp;&nbsp;&nbsp;'.date('H:i:s', strtotime($data['Fecha_Documento'])).'</h5>
            <h4 style="margin:5px 0 0 0;font-size:8px;line-height:8px;">F. Venc.:' . $this->fecha($cliente["Fecha_Pago"]) . '</h4>
        ...
        $nombre_paciente = trim(explode('-', $cliente["Nombre_Paciente"])[0]);
                ...
                <tbody>
                    <tr>
                        ...
                        <td style="width:150px;">
                            <img src="' . ($cliente["Codigo_Qr"] == '' ? $_SERVER["DOCUMENT_ROOT"] . 'assets/images/sinqr.png' : $_SERVER["DOCUMENT_ROOT"] . 'ARCHIVOS/FACTURACION_ELECTRONICA/' . $cliente["Codigo_Qr"]) . '" style="max-width:100%;margin-top:-8px;" />
                        </td>
                    </tr>
                    <tr>
        ...
        if ($func['Firma'] != '') {
            $imagen = '<img src="' . "$_SERVER[DOCUMENT_ROOT]" . "DOCUMENTOS/" . $func["Identificacion_Funcionario"] . "/" . $func['Firma'] . '"  width="110"><br>';
        } else {
        ...
        $pie =
            '<table cellspacing="0" cellpadding="0" style="text-transform:uppercase;margin-top:4px;">
                <tr>
                ...
                <tr>
                    <td style="font-size:7px;width:778px;background:#f3f3f3;vertical-align:middle;padding:1px 5px;height:10px;">
                        <strong>Resolución Facturación:</strong> Autorizacion de Facturacion Electrónica # ' . $fact["Resolucion"] . ' Desde ' . $this->fecha($fact["Fecha_Inicio"]) . ' Hasta ' . $this->fecha($fact["Fecha_Fin"]) . ' Habilita Del No. ' . $fact["Numero_Inicial"] . ' Al No. ' . $fact["Numero_Final"] . ' Actividad economica principal 4645<br>
                    </td>
                </tr>
                <tr>
                    <td style="font-size:7px;width:778px;background:#c6c6c6;vertical-align:middle;padding:1px 5px;text-align:center;">
        ...
        $content = 
            '<page backtop="250px" backbottom="140px" ' . $marca_agua . '>
        ...
        try {
            ...
            $html2pdf = new HTML2PDF('L', array(217, 170), 'Es', true, 'UTF-8', array(2, 0, 2, 0));
