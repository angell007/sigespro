...
$query = 'SELECT F.Imagen, CONCAT_WS(" ",F.Nombres, F.Apellidos) AS Funcionario, F.Identificacion_Funcionario, (SELECT  IFNULL(SUM(TIMESTAMPDIFF(MINUTE,Inicio_Fase1,Fin_Fase1)),0 ) FROM Remision WHERE Fase_1=F.Identificacion_Funcionario AND  MONTH(Fecha) = MONTH(NOW()) ) as Fase1,
...
INNER JOIN Remision R 
ON NC.Id_Remision=R.Id_Remision
WHERE NC.Tipo="Remision" AND MONTH(R.Fecha) =MONTH(NOW()) AND (Fase_1=F.Identificacion_Funcionario OR  Fase_2=F.Identificacion_Funcionario) ),0) as Novedades,
(SELECT COUNT(*) FROM Remision WHERE (Fase_1=F.Identificacion_Funcionario OR Fase_2=F.Identificacion_Funcionario) AND  MONTH(Fecha) = MONTH(NOW()) ) as Total_Rem,  (SELECT COUNT(*) FROM Producto_Remision PR INNER JOIN Remision R ON PR.Id_Remision=R.Id_Remision WHERE Fase_1=F.Identificacion_Funcionario  AND  MONTH(Fecha) = MONTH(NOW()) ) as Total_Productos_1, (SELECT COUNT(*) FROM Producto_Remision PR INNER JOIN Remision R ON PR.Id_Remision=R.Id_Remision WHERE Fase_2=F.Identificacion_Funcionario  AND  MONTH(Fecha) = MONTH(NOW()) ) as Total_Productos_2
...
foreach ($funcionarios as $item) {$i++;
    ...
    if(strpos($fase1,$item['Identificacion_Funcionario'])!==false){
        if($mayor_fase1_cantidad!=0 && $item['Total_Productos_1']>0){
            $promedio=$item['Fase1']/$item['Total_Productos_1'];
            $funcionarios[$i]['Promedio']=number_format($promedio,2,".","");
            ...
            $funcionarios[$i]['Promedio']=number_format(0,0,".","");
            $productividad=0;
            $porcentaje=0;
        }
        $funcionarios[$i]['Productividad']=number_format($productividad,0,"","");
        $funcionarios[$i]['Porcentaje']=number_format($porcentaje,2,".","");
        $pesonal_fase1[]=$funcionarios[$i];
    }else{
