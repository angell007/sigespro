...
$query = 'SELECT Count(Lote) as Conteo, Lote, Id_Producto, Fecha_Vencimiento, Id_Punto_Dispensacion
FROM Inventario 
WHERE Id_Bodega=0
GROUP BY Lote, Id_Producto, Id_Punto_Dispensacion
HAVING Conteo > 1  
...
foreach($lotes as $lote){ $i++;
   echo $i.")  ".$lote["Conteo"]." - ".$lote["Lote"]." - ".$lote["Id_Producto"]." - ".$lote["Id_Punto_Dispensacion"]."<br>"; 
   $query = 'SELECT Id_Inventario, Cantidad, Lote, Fecha_Vencimiento
            FROM Inventario WHERE Id_Producto = '.$lote["Id_Producto"].' AND Lote LIKE "'.$lote["Lote"].'" AND Id_Punto_Dispensacion = '.$lote["Id_Punto_Dispensacion"].' ORDER BY Id_Inventario  ASC';
        $oCon= new consulta();
        $oCon->setTipo('Multiple');
        $oCon->setQuery($query);
       ...
       /*  if(count($inventarios)==2){
            $query="UPDATE Producto_Dispensacion SET Id_Inventario=".$inventarios[1]['Id_Inventario']." WHERE Id_Inventario=".$inventarios[0]['Id_Inventario'];
            $oCon= new consulta();
            $oCon->setQuery($query);
            $oCon->createData();     
            unset($oCon);
        ...
        if(count($inventarios)>1){
            ...
            $query="UPDATE Producto_Dispensacion SET Id_Inventario=".$inventarios[0]['Id_Inventario']." WHERE Id_Inventario=".$inventarios[1]['Id_Inventario'];
            $oCon= new consulta();
            $oCon->setQuery($query);
            //$oCon->createData();     
            unset($oCon); 
            
              $query="UPDATE Producto_Remision SET Id_Inventario=".$inventarios[0]['Id_Inventario']." WHERE Id_Inventario=".$inventarios[1]['Id_Inventario'];
             // echo $query."<br>";
            $oCon= new consulta();
            $oCon->setQuery($query);
            //$oCon->createData();     
            ...
            $oItem=new complex('Inventario','Id_Inventario',$inventarios[0]['Id_Inventario']);
            ...
            $oItem->Id_Inventario_Eliminado=$inventarios[1]['Id_Inventario'];
            $oItem->Id_Inventario=$inventarios[0]['Id_Inventario'];
            //$oItem->save();
            unset($oItem);
