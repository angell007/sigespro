<?php
    ...
    A.Id_Auditoria,    
    A.Id_Dispensacion,
    D.Codigo as DIS,(SELECT CONCAT(S.Nombre," - ",T.Nombre) FROM Tipo_Servicio T INNER JOIN Servicio S ON T.Id_Servicio=S.Id_Servicio WHERE T.Id_Tipo_Servicio=A.Id_Tipo_Servicio ) as  Servicio,
    A.Archivo, 
    ...
    WHERE A.Estado="Pre Auditado" AND A.Id_Dispensacion IS NOT NULL AND  DATE(A.Fecha_Preauditoria)>"2019-04-14"
...
ON A.Id_Dispensacion=D.Id_Dispensacion
INNER JOIN (SELECT PD.Id_Punto_Dispensacion, D.Id_Departamento FROM Punto_Dispensacion PD INNER JOIN Departamento D ON PD.Departamento=D.Id_Departamento) PT ON A.Punto_Pre_Auditoria=PT.Id_Punto_Dispensacion
WHERE A.Estado="Pre Auditado" AND A.Id_Dispensacion IS NOT NULL  AND  DATE(A.Fecha_Preauditoria)>"2019-04-14" AND D.Estado_Dispensacion!="Anulada"
'.$condicion;    
