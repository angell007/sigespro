...
var_dump($listado_inventario);exit;
 */
...
foreach ($listado_inventario as $res) {
  ...
  // filtrar los ids de los documentos para luego cambiarle el estados
  if (!strpos($ids_docs_inventarios, $res['Id_Doc_Inventario_Fisico_Punto'])) {
    $ids_docs_inventarios .= ' ' . $res['Id_Doc_Inventario_Fisico_Punto'] . ' ,';
  }
  ...
  if (isset($res['Cantidad_Auditada']) && $res['Cantidad_Auditada'] != '') {
    $cantidad = number_format($res["Cantidad_Auditada"], 0, "", "");
     ActualizarProductoDocumento($res['Id_Producto_Doc_Inventario_Fisico_Punto'],$cantidad);
  }else{
...
$oItem->Id_Punto_Dispensacion = $listado_inventario[0]['Id_Punto_Dispensacion'];
...
$query2 = 'UPDATE Doc_Inventario_Fisico_Punto
 SET Estado ="Terminado", Fecha_Fin="' . date('Y-m-d H:i:s') . '" , Funcionario_Autorizo=' . $funcionario . ',
     Id_Inventario_Fisico_Punto_Nuevo=' . $inventario . '
 WHERE  Id_Doc_Inventario_Fisico_Punto IN (' . $ids_docs_inventarios . ')';
...
function ActualizarProductoDocumento($id_producto_doc_inventario,$cantidad){
 ...
 $query = 'UPDATE Producto_Doc_Inventario_Fisico_Punto SET Cantidad_Auditada ='.$cantidad .' , Funcionario_Cantidad_Auditada = ' .$funcionario.' 
        WHERE Id_Producto_Doc_Inventario_Fisico_Punto = '. $id_producto_doc_inventario;
  $oCon = new consulta();
  $oCon->setQuery($query);
  $oCon->createData();
